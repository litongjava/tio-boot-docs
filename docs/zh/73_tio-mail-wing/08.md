...
* 将接收的邮件内容持久化到数据库（通过 `MailboxService`）
...
`SmtpService` 中实现了各个 SMTP 命令的处理，并借助 `MwUserService` 和 `MailboxService` 完成用户认证与邮件持久化。
...
* **handleMail** / **handleRcpt** / **handleData** / **handleDataReceiving**：处理邮件事务，最终调用 `mailboxService.saveEmail(...)` 将邮件存入数据库。
* **handleRset**：重置当前邮件事务。
* **handleQuit**：关闭会话。
