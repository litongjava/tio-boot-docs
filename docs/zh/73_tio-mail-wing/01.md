...
3.  **测试**
    ...
    *   **客户端**：配置 Foxmail 或 Thunderbird 客户端，使用 POP3 协议连接的服务器。
...
2.  **实现命令逻辑 (状态流转)**
    ...
    *   `AUTH LOGIN`: 进行认证。这是客户端发信时必须的。需要处理 Base64 编码的用户名和密码。
...
3.  **测试**
    ...
    *   **客户端**：配置 Foxmail/Outlook，使用的服务器作为 SMTP 服务器向外发信（或给自己发信）。
    *   **外部服务器**：从的 QQ 邮箱或 Gmail 向在 `tio-mail-wing` 上创建的账户（如 `user@yourdomain.com`）发送一封邮件，看能否成功接收。
...
1.  **实现 SSL/TLS (STARTTLS)**
    ...
    *   `tio-boot` 支持 SSL/TLS。需要在 `AioHandler` 中捕获 `STARTTLS` 命令，然后调用 `Tio.startSSL(channelContext, ...)` 方法来升级连接。
    *   需要一个 SSL 证书（开发阶段可以使用自签名证书）。
...
2.  **部署**
    ...
    *   为了让公网能访问，需要一台有公网 IP 的服务器，并配置好域名和 DNS（MX 记录指向的服务器 IP）。
