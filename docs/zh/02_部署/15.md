...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <properties>
        ...
        <main.class>nexus.io.full.thin.HelloApp</main.class>
...
```java
package nexus.io.full.thin.controller;
...
- **Windows 启动**：
  ...
  **示例**：
  ...
  ```shell
  java -Xverify:none -cp config;lib\*;static nexus.io.full.thin.HelloApp
  ```
...
- **指定启动参数**：
  ...
  ```shell
  java -Xverify:none -cp ./config:./lib/*:./static nexus.io.full.thin.HelloApp --server.port=1003
  ```
...
c5 = "cd /data/apps/tio-boot-full-thin-demo01/tio-boot-full-thin-demo01-1.0.0 && docker run -dit --name tio-boot-full-thin-demo01 --restart=always --net=host -v $(pwd):/app -w /app -e TZ=Asia/Shanghai -e LANG=C.UTF-8 litongjava/jdk:8u391-stable-slim java -Xverify:none -cp ./config:./lib/*:./static nexus.io.full.thin.HelloApp"
...
ExecStart=/usr/java/jdk1.8.0_411/bin/java -Xverify:none -cp ./config:./lib/*:./static nexus.io.max.blog.MaxBlogApp --app.env=test
...
[Service]
ExecStart=/opt/tio-hello/tio-hello-1.0/tio.sh start nexus.io.full.thin.HelloApp
ExecStop=/opt/tio-hello/tio-hello-1.0/tio.sh stop
ExecReload=/opt/tio-hello/tio-hello-1.0/tio.sh restart nexus.io.full.thin.HelloApp
Type=forking
PrivateTmp=true
