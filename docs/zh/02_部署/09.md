...
2. [服务端安装](#服务端安装)
3. [部署方法](#部署方法)
   - [Fastjar + Docker](#fastjar--docker)
     - [打包配置文件 `.build.txt`](#打包配置文件-buildtxt)
     - [部署配置文件 `.deploy.toml`](#部署配置文件-deploytoml)
     - [执行部署命令](#执行部署命令)
     - [运行流程](#运行流程)
     - [部署成功后的反馈信息](#部署成功后的反馈信息)
     - [完整的部署文件示例](#完整的部署文件示例)
   - [Fastjar + Systemctl](#fastjar--systemctl)
     - [Service 文件配置](#service-文件配置)
     - [打包配置文件 `.build.txt`](#打包配置文件-buildtxt-1)
     - [部署配置文件 `.deploy.toml`](#部署配置文件-deploytoml-1)
4. [工具源码](#工具源码)
...
c1 = "[ -d /data/apps/max-blog-app-backend ] && cp -r /data/apps/max-blog-app-backend backup/max-blog-app-backend-backup-$(date +'%Y%m%d_%H%M%S')"
c2 = "mkdir -p /data/apps/max-blog-app-backend"
c3 = "mkdir -p backup"
