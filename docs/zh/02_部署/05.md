...
2. [安装 Maven](#安装-maven)
3. [克隆项目](#克隆项目)
4. [使用 Java 命令启动](#使用-java-命令启动)
5. [使用 Docker 启动](#使用-docker-启动)
6. [封装成镜像](#封装成镜像)
7. [使用 Builder 和 Runner](#使用-builder-和-runner)
8. [中文支持与北京时区](#中文支持与北京时区)
9. [使用二进制文件启动](#使用二进制文件启动)
...
为了使用二进制文件启动，推荐移除 `hotswap-classloader` 依赖，并使用 `TioApplication` 启动应用。示例代码如下：
