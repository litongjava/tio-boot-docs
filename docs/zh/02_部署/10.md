...
3. [打包](#打包)
   - [配置 Maven](#配置-maven)
     ...
     - [assembly-full.xml 配置](#assembly-fullxml-配置)
     - [assembly-thin.xml 配置](#assembly-thinxml-配置)
   - [执行打包命令](#执行打包命令)
   - [启动应用](#启动应用)
4. [部署](#部署)
   - [使用 `deploy` 工具自动化部署](#使用-deploy-工具自动化部署)
     - [配置文件说明](#配置文件说明)
       - [.build.txt 配置](#buildtxt-配置)
       - [.deploy.toml 配置](#deploytoml-配置)
     - [部署到 Docker](#部署到-docker)
     - [部署到 systemctl](#部署到-systemctl)
   - [配置 Systemd 服务](#配置-systemd-服务)
     - [创建 Service 文件](#创建-service-文件)
     - [启动并启用服务](#启动并启用服务)
   - [使用 Shell 脚本管理应用](#使用-shell-脚本管理应用)
     - [启动脚本 `tio.sh`](#启动脚本-tiosh)
     - [配置 Service 文件](#配置-service-文件-1)
5. [工具源码](#工具源码)
6. [结语](#结语)
