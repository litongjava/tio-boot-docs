...
- 生产环境 (production)：在生产环境中，使用 maven-assembly-plugin 插件来打包应用程序。这种配置支持生成胖包或瘦包，根据需求生成不同的部署包。
...
mvn clean package -DskipTests -Pproduction -Dassembly=full
```
...
```
set JAVA_HOME=D:\java\jdk1.8.0_121
mvn clean package -DskipTests -Pproduction -Dassembly=thin
```
...
p = "123456"
b = ".build.txt"
file = "target/tio-boot-full-thin-demo01-demo01-1.0.0.zip"
d = "unzip/tio-boot-full-thin-demo01"
c1 = "[ -d /data/apps/tio-boot-full-thin-demo01 ] && cp -r /data/apps/tio-boot-full-thin-demo01 backup/tio-boot-full-thin-demo01-backup-$(date +'%Y%m%d_%H%M%S')"
c2 = "mkdir -p /data/apps/tio-boot-full-thin-demo01"
c3 = "cp -r unzip/tio-boot-full-thin-demo01/* /data/apps/tio-boot-full-thin-demo01/"
c4 = "if [ $(docker ps -q -f name=tio-boot-full-thin-demo01-demo01) ]; then docker stop tio-boot-full-thin-demo01 && docker rm -f tio-boot-full-thin-demo01; fi"
c5 = "cd /data/apps/tio-boot-full-thin-demo01/tio-boot-full-thin-demo01-demo01-1.0.0 && docker run -dit --name tio-boot-full-thin-demo01 --restart=always --net=host -v $(pwd):/app -w /app -e TZ=Asia/Shanghai -e LANG=C.UTF-8 litongjava/jdk:8u391-stable-slim java -Xverify:none -cp ./config:./lib/*:./static com.litongjava.full.thin.HelloApp"
