...
```java
package nexus.io.websockify.handler;
...
```java
package nexus.io.websocket.handler;
...
```
2024-09-01 01:02:27.619 [tio-group-3] INFO  nexus.io.websocket.handler.HelloWebSocketHandler.handshake - 收到来自 127.0.0.1 的 WebSocket 握手请求：GET /hello?name=Tong%20Li HTTP/1.1
```
...
```
2024-09-01 01:03:56.925 [tio-group-6] INFO  nexus.io.websocket.handler.HelloWebSocketHandler.onText - 路径：/hello，收到消息：hi
```
...
```
2024-09-01 01:05:12.123 [tio-group-4] INFO  nexus.io.websocket.handler.HelloWebSocketHandler.onClose - 连接关闭，用户：Tong Li
```
...
public interface IWebSocketHandler {
  ...
  /**
   * 当收到 Opcode.BINARY 消息时执行该方法。也就是说，如果的 WebSocket 是基于 BINARY 传输的，就会调用此方法。
   *
  ...
  /**
   * 当收到 Opcode.TEXT 消息时执行该方法。也就是说，如果的 WebSocket 是基于 TEXT 传输的，就会调用此方法。
   *
   * @param wsRequest WebSocket 请求对象
   * @param text 消息的文本内容
   * @param channelContext 当前连接的上下文信息
