...
2. **实现对 `NAMESPACE` 命令的响应**
   在的 `ImapServerAioHandler`（或相应的命令分发部分）里，增加：
...
> **Tip:** 如果还想让 Gmail 能识别 “所有邮件”/“已删除邮件” 等特殊文件夹，可以再实现 [SPECIAL-USE 扩展](https://tools.ietf.org/html/rfc6154)（比如返回 `\All`, `\Trash` 等），或实现它的旧版 `XLIST` 扩展，但这不是崩溃的根本原因。
