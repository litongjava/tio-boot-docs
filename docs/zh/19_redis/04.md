...
本例的 Spring 项目配置中，`RedisTemplate` 使用 `JdkSerializationRedisSerializer` 进行序列化，它会将对象通过 Java 标准的序列化机制转换为字节数组，因此在 Redis 中会出现类似于 `\xAC` 这样的不可读二进制数据。
...
import nexus.io.redis.Redis;
import nexus.io.redis.RedisPlugin;
import nexus.io.redis.serializer.KeyValueJdkSerializer;
import nexus.io.tio.utils.json.JsonUtils;
