...
Caffeine 本身不会将数据存储到 Redis 中。如果想要将数据缓存到 Redis 中，需要使用专门为 Redis 设计的缓存解决方案，如 Spring Cache with Redis 或 Jedis。这些工具和库允许直接与 Redis 数据库交互，将数据存储在 Redis 中并从中检索。
...
## 结合使用 Caffeine 和 Redis

要创建一个服务，整合 Caffeine 和 Redisson 缓存，可以遵循以下步骤。此服务将首先检查 Caffeine 缓存中是否存在所需数据。如果未找到，它将检查 Redisson 缓存，并可能将从 Redisson 检索的数据放入 Caffeine 缓存以加快后续访问的速度。

##### 编写一个整合 Caffeine 和 Redisson 的服务类

```java
package nexus.io.tio.boot.hello.services;
...
**使用服务**: - 可以在的应用程序中的其他部分，如控制器或业务逻辑层中，注入或实例化 `CacheService` 类，并通过它来处理缓存逻辑。

这个服务提供了一个简单的方式来整合两种不同类型的缓存，利用了 Caffeine 的高性能本地缓存能力和 Redisson 的分布式缓存能力。通过这种方式，可以提高数据检索的效率和应用程序的整体性能。
...
import nexus.io.jfinal.aop.Aop;
import nexus.io.tio.boot.hello.services.CacheService;
...
该示例没有跨节点本地缓存失效通知，其他进程中的 Caffeine 可能保留旧值。实际项目需设置合理的本地 TTL，并按业务要求设计失效通知或版本校验；仅依次更新两层缓存不等于强一致性。
