...
本页 lock/withLock 示例用于解释 API。2026-09-16 核对的 java-db 1.5.9 源码中，unlock 使用独立的 GET 和 DEL，比较令牌与删除之间存在竞争窗口；withLock 的 finally 会调用 unlock，但不会消除这一问题，也没有自动续租。不要据此承诺超时或故障情况下的互斥正确性。
