...
> 依据 project-nexus/java-db 1.5.9 本地源码核对类型名和回调签名（2026-09-16）。`Redis.use()` 返回 `RedisDb`；分布式锁限制见 [使用示例](./03.md#分布式锁的实现边界)。
...
- **`void addCache(RedisDb cache)`**

  - 向 `Redis` 类中添加一个新的 Redis 缓存实例。
  - 参数：
    - `cache`: 要添加的 `RedisDb` 实例，不能为 `null`。
  - 异常：如果缓存实例为 `null` 或缓存名称已经存在，则抛出 `IllegalArgumentException`。

- **`RedisDb removeCache(String cacheName)`**

  - 从 `Redis` 类中移除指定名称的 Redis 缓存实例。
  - 返回值：被移除的 `RedisDb` 实例。如果指定名称的缓存不存在，则返回 `null`。
...
- **`RedisDb use()`**
  ...
  - 获取主缓存 `mainCache` 实例。
  - 返回值：当前的主缓存实例 `RedisDb`。

- **`RedisDb use(String cacheName)`**

  - 获取指定名称的 Redis 缓存实例。
  - 返回值：对应名称的 `RedisDb` 实例。如果指定名称的缓存不存在，则返回 `null`。
...
- **`<T> T callback(IRedisCallback<T> callback)`**

  - 使用指定的回调函数操作主缓存实例的 Jedis 对象。
  - 返回值类型：`T`，即回调函数的返回值。

- **`<T> T callback(String cacheName, IRedisCallback<T> callback)`**
...
`RedisDb` 类提供了对 Redis 操作的封装，涵盖了键值存储、哈希、列表、有序集合等常见 Redis 操作，并提供了丰富的 API，支持常见的缓存和分布式锁功能。该类基于 Jedis 库进行操作，支持序列化键和值。

#### 构造函数

- **`RedisDb(String name, JedisPool jedisPool, ISerializer serializer, IKeyNamingPolicy keyNamingPolicy)`**
  - 构造一个 `Cache` 实例。
  - `name`: 缓存名称。
