...
JSQL(JSON To table SQL):是一种以 JSON 为载体的结构化查找语言,用于
...
- http://localhost:10051/table/system_users/get?username=admin
  这这个请求示例中 system_users 被称为 data table name 数据表名称
...
数据字段指令`columns` 用于指定查询结果中要返回的数据字段。默认情况下，如果不指定 `columns`，查询会返回数据表包含的所有数据字段。通过使用 `columns` 指令，可以明确指出需要的特定字段，从而优化查询结果和性能。
