...
* 成功请求记录表：`tio_llm_usage`
* 失败请求记录表：`tio_llm_generate_failed`
* 统一调用入口：`UniPredictService`
* 成功落库：`TioLlmUsageDao.saveUsage`
* 失败落库：`TioLlmGenerateFailedDao.save`
...
### 1.1 成功调用表 tio_llm_usage

该表用于记录每一次成功生成的调用信息，用于统计、审计与成本分析。

```sql
drop table if exists tio_llm_usage;
CREATE TABLE tio_llm_usage (
...
### 1.2 失败调用表 tio_llm_generate_failed

该表用于保存每一次失败请求的完整上下文，核心价值是：保留“真实请求 JSON、响应 JSON 与堆栈”，让排障不依赖复现。

```sql
drop table if exists tio_llm_generate_failed;
CREATE TABLE tio_llm_generate_failed (
  id BIGINT PRIMARY KEY,
  group_id bigint,
  group_name varchar,
  task_id bigint,
...
import nexus.io.manim.dao.TioLlmGenerateFailedDao;
import nexus.io.manim.dao.TioLlmUsageDao;
  ...
  public UniChatResponse generate(UniChatRequest uniChatRequest) {
    ...
    for (int i = 0; i < 10; i++) {
      try {
        ...
        Aop.get(TioLlmUsageDao.class).saveUsage(uniChatRequest, uniChatResponse, (endTime - currentTime));
        ...
        try {
          Aop.get(TioLlmGenerateFailedDao.class).save(uniChatRequest, e, stackTrace);
        } catch (Exception e1) {
...
## 3. 成功落库：TioLlmUsageDao.saveUsage
...
import nexus.io.tio.boot.admin.dto.TioLlmUsage;
...
## 4. 失败落库：TioLlmGenerateFailedDao.save
...
import nexus.io.db.base.DbBase;
import nexus.io.exception.GenerateException;
import nexus.io.tio.boot.admin.dto.TioLlmGenerateFailed;
import nexus.io.tio.utils.snowflake.SnowflakeIdUtils;
...
* 将其写入 `tio_llm_generate_failed`，即可做到：
