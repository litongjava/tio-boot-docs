...
* DashScope 的转写事件：
  ...
  * 转成你现有的 `WsVoiceAgentResponseMessage(type="transcript_in/out")` 发给前端

这样 `index.html/app.js/mic-worklet.js` 基本不用改。

### 4.2 `audio_end` 的含义要重新定义

你现在的 `audio_end` 在 Gemini Live 里是“告诉模型音频流结束”。
在 DashScope 里：
...
* 保留 `RealtimeBridgeCallback` 接口不变（复用你现有 WebSocket 下发逻辑）
...
public class QwenOmniRealtimeBridge implements RealtimeModelBridge {
  ...
  /**
   * Manual 模式：把你现有的 audio_end 映射为 commit + createResponse
   * 如果你使用 server_vad，可不调用这个方法。
   */
  ...
  /**
   * 文本输入：SDK 里有的版本支持发文本项；若你只做语音助手，可先不实现。
   * 如果你确实要支持“输入框发文本”，建议用 Realtime 的对话项事件（conversation.item.create）
...
* 模型下行的 `response.audio.delta` 是 **Base64**，后端解码成 bytes，再走你现有的 `callback.sendBinary(bytes)` 给浏览器播放
* `response.done` 映射为你前端习惯的 `turn_complete`

`commitAndCreateResponse()` 用于 Manual 模式；如果你走 server_vad 模式（推荐通话），通常不用调用。Manual 模式的 `commit` + `response.create` 语义与官方说明一致。([AlibabaCloud][5])
...
你可以在前端 `ws.onmessage` 的 JSON 分支里加：

* 收到 `speech_started` 时：清空播放队列（你现在用 `nextPlayTime` 排队，可以加一个“清空/重置”逻辑，把 `nextPlayTime = playCtx.currentTime + 0.01`，并把未播的 chunk 计数清掉）
...
你现在看到的“模型音频转写”通常对应这些服务端事件：

* `response.audio_transcript.delta / done`（助手输出音频的字幕）
  而“用户输入音频转写”对应事件是：
* `conversation.item.input_audio_transcription.completed`

如果你没配置 `InputAudioTranscription=gummy-realtime-v1`，或者在 Manual 模式下没 commit，就很容易只出现助手字幕，不出现用户转写。
...
示例（按 SDK 实际方法名二选一；你用 IDE 看 builder 上到底是哪个）：
...
conversation.updateSession(OmniRealtimeConfig.builder()
    ...
    .inputAudioTranscription("gummy-realtime-v1")   // 如果你的 SDK builder 有这个方法
...
Manual 模式下：**输入音频的转写通常在你 commit 之后才会发生**。([Alibaba Cloud][1])
所以流程要是：
...
---
[1]: https://www.alibabacloud.com/help/en/doc-detail/2950053.html "
 Java SDK, Qwen-Omni, real-time, multimodal - Alibaba Cloud Model Studio - Alibaba Cloud Documentation Center


## ref
[1]: https://www.alibabacloud.com/help/en/model-studio/qwen3-livetranslate-flash-realtime?utm_source=chatgpt.com "Real-time audio and video translation - Qwen"
[2]: https://mvnrepository.com/artifact/com.alibaba/dashscope-sdk-java/2.20.9?utm_source=chatgpt.com "com.alibaba » dashscope-sdk-java » 2.20.9"
[3]: https://www.alibabacloud.com/help/en/model-studio/install-sdk?utm_source=chatgpt.com "Alibaba Cloud Model Studio:Install the SDK"
[4]: https://www.alibabacloud.com/help/en/model-studio/qwen-real-time-speech-recognition?utm_source=chatgpt.com "Real-time speech recognition - Qwen"
[5]: https://www.alibabacloud.com/help/en/model-studio/client-events?utm_source=chatgpt.com "Alibaba Cloud Model Studio:Client events"
[6]: https://www.alibabacloud.com/help/en/model-studio/omni-realtime-interaction-process?utm_source=chatgpt.com "Real-time multimodal interaction flow"
[7]: https://bailian.console.aliyun.com/cn-beijing/?tab=doc#/doc/?type=model&url=2880812