...
本文档提供了如何将 tio-boot-server 集成到使用 spring-boot-starter-webflux 的项目中。默认情况下，Spring WebFlux 使用 Netty 作为网络处理层。本文档将引导如何将 tio-boot-server 作为替代品整合到的 Spring WebFlux 应用中。
...
import nexus.io.hotswap.wrapper.spring.boot.SpringApplicationWrapper;
import nexus.io.jfinal.aop.annotation.AComponentScan;
import nexus.io.tio.boot.spring.SpringBootArgs;
import nexus.io.tio.boot.spring.TioBootServerAutoConfiguration;
...
通过这种方式，可以将 tio-boot-server 与 Spring WebFlux 结合使用，利用 tio-boot 提供的高性能网络处理能力，同时享受 Spring WebFlux 的响应式编程模型和丰富的开发特性。
