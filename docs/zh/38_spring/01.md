...
import nexus.io.db.activerecord.Row;
import nexus.io.jfinal.aop.Aop;
import nexus.io.spring.boot.tio.boot.demo01.services.UserService;
...
import nexus.io.db.activerecord.Row;
import nexus.io.jfinal.aop.Aop;
import nexus.io.spring.boot.tio.boot.demo01.SpringBootWebApplication;
...
- 使用 `@AopBefore` 注解的方法会在每个测试方法执行前启动 TioApplication（通过传递 `--tio.noServer=true` 参数，不启动 Tio Boot 的 HTTP 服务）。
