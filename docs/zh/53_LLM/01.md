...
```xml
<java-openai.version>1.3.1</java-openai.version>
...
`java-openai` 按“平台/能力域”组织代码，每个目录大体代表一个厂商平台、一个能力模块或一个业务通用层。给出的目录如下（节选说明）：
...
* `mcp`
  一般用于“工具协议/工具调用”的扩展模块（若这里指的是 Model Context Protocol 或类似思想），用来把外部能力以统一方式暴露给模型或业务。
...
`UniChatRequest` 是业务侧与 `UniChatClient` 之间的统一契约。可以把它当作“跨平台 Chat 请求 DTO”。
...
对于只想发送一条或几条用户消息的简单场景，可以直接使用快捷方法：
...
`setUserPrompts(String...)` 会将每个非空字符串包装为一条 `user` 角色的 `UniChatMessage`，免去手工构造 `messages` 列表。

## 4. 生成参数

* `temperature`：随机性/发散程度
* `max_tokens`：最大输出 token 限制（不同平台命名不同，此处归一）
* `responseFormat`：响应格式类型（例如 text/json 等，按内部枚举 `ChatResponseFormatType` 组织）
...
* `enable_search`：是否开启搜索能力（对接支持检索增强的供应商或的 `search/*` 模块）

## 7. 缓存/复用（可选）

* `cachedId`：用于复用某次缓存结果或上下文（具体语义由的实现定义，比如命中缓存直接返回、或复用某个会话上下文）

## 8. 工具/函数调用（可选）

* `tools`：`List<ChatRequestTool>`，统一描述可供模型调用的工具/函数定义。
  业务侧只需要按统一结构声明工具，底层适配层会根据目标平台（OpenAI / Claude / Gemini 等）转换为对应协议中的 `tools` / `functions` / `tool_config` 等字段。

配合 `UniChatMessage` 的工具消息构造方法，可以表达完整的多轮工具调用上下文：

* `UniChatMessage.buildAssistantWithToolCalls(List<ToolCall>)`
  模型在某轮发起的工具调用（assistant 消息携带 `tool_calls`）。

* `UniChatMessage.buildTool(toolCallId, name, content)`
  工具执行结果回填到对话上下文（tool 角色消息）。
...
* `delta`
  ...
  在 SSE/WebSocket 场景中，通常会不断拿到 delta 并拼接成完整文本。
...
`ModelPlatformName` 中已经内置了大量平台常量，业务侧通过 `PlatformInput(platform, model)` 即可在它们之间无缝切换：

* 三大原生协议：`OPENAI` / `GOOGLE` / `ANTHROPIC`
* 谷歌云通道：`VERTEX_AI`
* 国内/区域聚合或企业平台：`VOLC_ENGINE`、`BAILIAN`、`TENCENT`、`MOONSHOT`、`MINIMAX`、`SILICONFLOW`、`CHAT_GLM`、`GITEE`
* 海外聚合平台：`OPENROUTER`、`ZENMUX`、`CEREBRAS`
* 私有化/自托管推理引擎：`OLLAMA`、`LLAMACPP`、`VLLM`、`SWIFT`、`TITANIUM`
* 中转/代理通道：`EXCHANGE_TOKEN`、`EXCHANGE_TOKEN_GOOGLE`、`EXCHANGE_TOKEN_ANTHROPIC`、`EXCHANGE_TOKEN_US`、`EXCHANGE_TOKEN_US_GOOGLE`、`EXCHANGE_TOKEN_US_ANTHROPIC`
* 其他扩展：`AIAPI`、`ELEVEN_LABS`、`AUTO`

`UniChatClient.generate(...)` 与 `UniChatClient.stream(...)` 会根据 `platform` 自动路由到对应的适配实现，并自动从环境变量读取 API Key 与 API URL（例如 `OPENROUTER_API_KEY` / `OPENROUTER_API_URL`、`VERTEX_AI_API_KEY` / `VERTEX_AI_API_URL` 等），业务侧无需重复配置。
...
* 业务侧使用 `UniChatResponse.delta` 拼接渲染，并在结束时读取 usage 等信息（具体以的实现为准）
