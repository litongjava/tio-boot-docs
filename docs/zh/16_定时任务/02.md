...
添加配置文件指定 xxl.job.admin.addresses 为的任务调度中心地址
...
import nexus.io.tio.boot.hello.job.MyJobHandler;
...
```
package nexus.io.tio.boot.hello.job;
...
可以手动想执行器发送 post 请求触发执行任务,发送的消息格式如下
