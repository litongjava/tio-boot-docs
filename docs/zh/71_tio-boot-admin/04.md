...
本章介绍**后台管理前端**及 ApiTable 组件。普通用户使用的 H5、微信小程序可以单独采用 uni-app 等技术，通过业务 API 接入同一后端。两类前端的接口权限不同，不能把后台 Token 或通用表接口直接用于普通用户页面。业务接口与联调约定见 [业务 API 与 H5 / 小程序联调](./17.md)。

通过上述步骤，我们已经成功搭建了一个简单的后台管理系统的框架。`tio-boot-admin` 提供了丰富的功能接口，方便后续开发和扩展。接下来，需要配合前端项目进行界面开发和功能对接。

## tio-boot-admin-react

推荐使用 [`tio-boot-admin-react`](https://github.com/litongjava/tio-boot-admin-react) 作为前端解决方案，该项目基于 React 框架，提供了丰富的组件和示例，能够与 `tio-boot-admin` 无缝集成，帮助快速构建现代化的后台管理界面。
...
2. **安装依赖**:
   ...
   ```
3. **配置接口地址**:
   根据后端 `server.context-path` 和端口号，配置前端项目中的 API 基础路径。例如上下文为 `/admin` 时，后台登录的外部地址为 `/admin/api/login/account`；Java 路由仍注册 `/api/login/account`。开发代理应明确是否保留前缀，避免重复拼接或意外删除 `/admin`。
...
4. **启动前端**:
   ...
   ```
5. **访问后台管理系统**:
   打开前端启动日志实际显示的地址。端口由前端项目配置和启动参数决定，不应固定假设为 `3000`。

以上安装与启动命令应与所检出的前端项目 `package.json`、`packageManager` 和锁文件一致。项目使用 pnpm 时，使用 `pnpm install --frozen-lockfile` 及对应的 `pnpm run <脚本名>`，不要同时生成 npm 与 pnpm 两套锁文件。镜像地址可放在项目 `.npmrc` 中，不必修改电脑的全局配置。

通过以上步骤，将拥有一个完整的后台管理系统，包括后端服务和前端界面，极大地提升开发效率和项目质量。
