...
```sql
CREATE TABLE IF NOT EXISTS tio_boot_admin_file_images (
...
先按 [文件上传](./05.md) 配置存储、凭据与文件表，并核对 [入门指南中的云文件路由](./01.md)。上传成功后保存经服务端校验的文件记录，失败时不写入虚构 URL。已有表须手动检查差异，IF NOT EXISTS 不会迁移字段。
