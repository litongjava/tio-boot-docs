...
> 本文由原 03.md 移至 16.md，仅供明确需要维护旧 Sa-Token 集成的项目参考。当前 tio-boot-admin 默认使用框架 Token/JWT 鉴权，标准接入无需添加 Sa-Token 依赖、配置类或表。请先阅读 [入门指南](./01.md) 和 [配置职责说明](./03.md)。本文保留旧版实现供参考，类名、表名与依赖需按旧项目实际版本核对；涉及 SQL 仍由用户手动执行。
