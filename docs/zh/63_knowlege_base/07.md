...
import nexus.io.maxkb.model.DocumentBatchVo;
import nexus.io.maxkb.model.KbDatasetModel;
import nexus.io.maxkb.service.DatasetDocumentVectorService;
import nexus.io.maxkb.service.MaxKbDatasetService;
import nexus.io.maxkb.service.MaxKbDatasetServiceHitTest;
import nexus.io.maxkb.service.MaxKbDocumentService;
...
可以将 `max_kb_embedding_cache` 表中的向量字段 `v` 与其他业务表进行关联，从而直接在数据库侧完成相似度计算。假设通过 `id` 字段关联，并基于该向量执行相似度检索，可以参考如下 SQL 查询示例：
...
import nexus.io.jfinal.aop.Aop;
import nexus.io.kit.RecordUtils;
