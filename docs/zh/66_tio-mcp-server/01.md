...
本文系统性讲解如何基于 **tio-boot** 从零实现一个符合 **MCP（Model Context Protocol）** 的 Server，并通过 **SSE（Server-Sent Events）** 与 MCP Client（如 openai-mcp）完成真实协议交互。文末保留了**完整代码**与**完整抓包协议内容**，便于对照实现细节与运行行为。
...
import nexus.io.mcp.context.McpRequestContext;
import nexus.io.mcp.exception.McpRpcException;
import nexus.io.mcp.model.JsonRpcRequest;
import nexus.io.mcp.model.JsonRpcResponse;
import nexus.io.mcp.model.McpInitializeParams;
import nexus.io.mcp.model.McpInitializeResult;
import nexus.io.mcp.model.McpMethod;
import nexus.io.mcp.model.McpToolsCallParams;
import nexus.io.mcp.model.McpToolsCallResult;
import nexus.io.mcp.model.McpToolsListResult;
import nexus.io.mcp.model.RpcError;
import nexus.io.mcp.server.IMcpServer;
...
```java
package nexus.io.kit.mcp;

import java.util.ArrayList;
import java.util.List;
import java.util.Map;

import nexus.io.kit.service.GoogleGeminiSearchService;
import nexus.io.kit.utils.CmdInterpreterUtils;
import nexus.io.kit.utils.PythonInterpreterUtils;
import nexus.io.mcp.model.McpContent;
import nexus.io.mcp.model.McpServerInfo;
import nexus.io.mcp.server.McpToolRegistry;
import nexus.io.mcp.server.RegisteredTool;
...
/**
 * 的具体服务器：只需提供名称/版本。 如需添加或覆盖工具，可覆写 registerAdditionalTools() 或单独覆盖某个 handler。
 */
@Slf4j
