...
接下来，使用 `java-db` 的 `nexus.io.db.activerecord.generator.Generator` 工具生成实体类代码。生成的实体类包括基础类 `BaseMaxKbApplicationAccessToken` 和主实体类 `MaxKbApplicationAccessToken`，分别负责基础的数据库字段映射和主键、表名的定义。以下是生成的实体类代码片段：

#### `MaxKbApplicationAccessToken.java`

```java
package nexus.io.maxkb.model;

import nexus.io.maxkb.model.base.BaseMaxKbApplicationAccessToken;
...
import nexus.io.maxkb.constant.AppConstant;
import nexus.io.maxkb.model.MaxKbApplicationAccessToken;
...
import nexus.io.maxkb.config.DbConfig;
