...
import nexus.io.table.model.TableInput;
import nexus.io.table.model.TableResult;
...
在 Java 等编程语言中，UUID（Universally Unique Identifier）通常表示为一个 128 位的值。UUID 包含两个部分：最显著位（most significant bits）和最不显著位（least significant bits），各占 64 位。的例子中，这两个部分分别是：

- `mostSignificantBits`: 2165688298263563317
- `leastSignificantBits`: -5614157302637544926

要从这两个值生成一个 UUID，可以使用 UUID 类中的构造函数，具体代码如下：
...
```java
package nexus.io.open.chat.services.sys;
...
import nexus.io.open.chat.services.sys.SysService;
