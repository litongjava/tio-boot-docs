...
public class NettyServerBootstrap {
  private int port;
  private NettyServerChannelHandler NettyServerChannelHandler;
  private EventLoopGroup boss = new NioEventLoopGroup();
  private EventLoopGroup worker = new NioEventLoopGroup();

  public NettyServerBootstrap(int port, NettyServerChannelHandler NettyServerChannelHandler) {
    this.port = port;
...
import com.litongjava.tio.utils.thread.ThreadUtils;
import com.litongjava.tio.web.hello.nettyserver.NettyServerChannelHandler;
...
public class NettyServerConfig {
  ...
  public void nettyServerBootstrap() {
    ...
    NettyServerChannelHandler NettyServerChannelHandler = new NettyServerChannelHandler();
...
- `NettyServerChannelHandler` 用于初始化通道。
...
- 构造函数: 接收端口号和 `NettyServerChannelHandler` 实例。
...
- `nettyServerBootstrap()`: 此方法创建并配置 `NettyServerBootstrap` 实例。它从环境配置中读取端口号，并在新线程中启动服务器。这是一个被 `@AInitialization` 注解标记的方法，用于在服务启动时 创建 `NettyServerBootstrap` 类的实例。

  - **初始化过程**：方法首先通过 `EvnUtils` 获取环境配置信息，从中读取服务器端口号。这种方式表明它使用了 JFinal 的 AOP 特性来注入依赖，这里是获取应用程序的环境设置。

  - **创建 `NettyServerBootstrap` 实例**：使用从环境配置中获取的端口号和一个新创建的 `NettyServerChannelHandler` 实例，初始化 `NettyServerBootstrap` 类的对象。
