...
## Recent标签是在什么时候清除的
根据 RFC 3501（IMAP4rev1）中对 `\Recent` 标志的定义，它并不是在第一次 SELECT 之后立即清除的，而是一个“会话级”新邮件标记，其清除时机如下：

1. **同一会话内的 SELECT/EXAMINE 不会清除它**
   当在同一个连接（session）里再次对同一信箱执行 SELECT 或 EXAMINE 命令，服务器**不**会把已经标记为 `\Recent` 的消息去掉这个标志——因为它还未结束这个会话。
...
3. **跨会话选信箱也会重置**
   如果在另一个客户端（或同一客户端的新连接）第一次 SELECT 该信箱，服务器同样会将所有旧的 `\Recent` 标记去掉，只在该新会话里，对自上次打开后新到达的邮件继续打上 `\Recent`。

总结：看到的“`SELECT` 之后没有清除”是符合规范的——`FETCH`、`STORE`、再次 `SELECT` 都不会影响 `\Recent`。只有发 `CLOSE`（或断开/LOGOUT），然后新会话里重新 `SELECT`，才会将先前的 `\Recent` 全部清掉。
...
* 如果的客户端最后一次执行的是 `SELECT server_error`，那么退出（`LOGOUT`）时就会清除 **server\_error** 邮箱上的 `\Recent`。
* 即便之前在同一会话里选过 `INBOX`，只要后来又选了 `server_error`，就只会清理后者的 `\Recent`。

要想在退出时清除 `INBOX` 的 `\Recent`，需要在 `LOGOUT` 前执行
