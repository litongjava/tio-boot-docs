...
本文将介绍如何使用 React 和 Ant Design 和 tio-boot-admin-react `ApiTableLong` 组件，在表格的列中添加一个操作按钮。点击按钮后触发网络请求，网络请求成功后刷新表格。同时展示了如何增加全局的按钮，实现类似功能。
...
export default () => {
  ...
  const onClickRequest = async (id: string) => {
    const hide = messageApi.loading("Translating...");
    try {
  ...
  const editContentAndPreview = {
    ...
    render: (text: any, record: { id: string }) => [
      <a key={`translate-${record.id}`} onClick={() => onClickRequest(record.id)}>
        Translate
      </a>,
    ],
  };
...
const editContentAndPreview = {
  ...
  width: 200,
  render: (text: any, record: { id: string }) => [
    <a key={`translate-${record.id}`} onClick={() => onClickBatchRequest(record.id)}>
      Translate
    </a>,
  ],
};
