...
<dependency>
  ...
  <artifactId>ai-agent-web</artifactId>
...
import nexus.io.ai.agent.config.AgentControllerConfiguration;
import nexus.io.ai.agent.handler.ApiChatAskHandler;
import nexus.io.ai.agent.handler.ApiChatHandler;
import nexus.io.ai.agent.handler.GeogebraChatHandler;
import nexus.io.ai.devops.bot.services.AiDevOptChatService;
...
import nexus.io.tio.boot.admin.config.TioAdminEhCacheConfig;
...
public class AdminAppConfig implements BootConfiguration {
  ...
  public void config() {
    ...
    if (r != null) {
      ...
      r.add("/api/v1/chat/ask", new ApiChatAskHandler(new AiDevOptChatService()));
    }
