...
下面是两个实例，展示如何使用 `ChatPacket` 类生成二进制数据并将其解析回 `ChatPacket` 对象。可以将这些实例直接应用到的 Java 项目中。
...
```java
package nexus.io.im.protobuf;

import nexus.io.im.protobuf.ChatPacketOuter.ChatPacket;
import nexus.io.im.protobuf.ChatPacketOuter.CommandType;
...
通过这两个实例，可以将 `ChatPacket` 对象与二进制数据互相转换，从而方便地在网络传输、数据存储等场景中使用 Protobuf 的高效序列化与反序列化特性。
...
import com.google.protobuf.InvalidProtocolBufferException;
import com.google.protobuf.util.JsonFormat;
import nexus.io.im.protobuf.ChatPacketOuter.ChatMessage;
import nexus.io.im.protobuf.ChatPacketOuter.ChatPacket;
import nexus.io.im.protobuf.ChatPacketOuter.CommandType;
import nexus.io.im.protobuf.ChatPacketOuter.TextContent;
...
public class ChatPacketToJsonExample {
  ...
  public static void main(String[] args) {
    // 创建一个 ChatPacket 对象
    TextContent textContent = nexus.io.im.protobuf.ChatPacketOuter.TextContent.newBuilder()
        //
        .setText("Hello, world!").build();

    ChatMessage chatMessage = nexus.io.im.protobuf.ChatPacketOuter.ChatMessage.newBuilder()
        //
