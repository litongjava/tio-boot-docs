...
- 要使用 `curl` 接收并播放 `Transfer-Encoding: chunked` 编码的 PCM 16kHz 音频文件，将 `curl` 的输出重定向到一个播放程序。这可以通过在 Unix 系统上使用 `arecord` 和 `aplay` 来实现。`arecord` 用于录制音频，`aplay` 用于播放音频。

### 步骤

1. **服务器端准备**
   确保服务器端能够以 `chunked` 编码方式返回 PCM 16kHz 音频数据。例如，使用 Node.js 实现：
   ...
   const server = http.createServer((req, res) => {
     if (req.url === "/audio") {
       ...
       // 读取并分块发送 PCM 音频数据
       const audioStream = fs.createReadStream("path/to/pcm16k_audio.pcm");
...
2. **客户端使用 `curl` 接收并播放音频**
   ...
   ```bash
   curl http://localhost:3000/audio | aplay -f S16_LE -r 16000 -c 1
   ```

   解释：

   - `curl http://localhost:3000/audio` 从服务器获取音频数据。
...
2. **使用 `curl` 和 `ffplay` 播放音频**
   ...
   ```powershell
   curl http://localhost:3000/audio | ffplay -f s16le -ar 16000 -ac 1 -
   ```
