...
1. 添加依赖：首先，需要在项目的`pom.xml`中添加 Caffeine 的依赖项。tio-boot 并没有内置了 caffeine-2.9.3 依赖
...
2. 配置缓存：在的 Java 配置类中，可以创建一个 Caffeine 缓存实例。例如：
...
3. 使用缓存：一旦缓存被配置，就可以在应用中使用它来存储和检索数据。

```java
package com.earndao.telegram.controller;
...
`CaffeineUtils` 是一个tio-uitls内置的用于管理缓存的工具类，支持创建和获取 Caffeine 缓存实例。它提供了两种方式：
...
下面通过一个控制器示例，展示如何在实际应用中使用 `CaffeineUtils`。CaffeineUtils.getCache默认是5分钟过期
...
如果想在创建缓存时使用自定义配置，可以调用 `createLoadingCache` 方法。例如：
...
`CaffeineUtils` 提供了一个简单易用的工具类，帮助开发者方便地管理 Caffeine 缓存。通过它，可以轻松实现缓存的自动创建、获取、和管理，提升应用的性能。
