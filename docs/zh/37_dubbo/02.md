...
```java
package nexus.io.tio.duboot.demo001.service;
...
```java
package nexus.io.tio.duboot.demo001.service.impl;

import nexus.io.jfinal.aop.annotation.AService;
import nexus.io.tio.duboot.demo001.service.HelloService;
...
```java
package nexus.io.tio.duboot.demo001.controller;
...
```java
package nexus.io.tio.duboot.demo001;
...
当访问`http://localhost/`时，HTTP 请求会被路由到`IndexController`的`index()`方法。由于`helloService`已经被注入为`HelloServiceImpl`的实例，因此`helloService.sayHello("Tong Li")`会调用`HelloServiceImpl`的`sayHello`方法，并返回`"Hello Tong Li"`。最终，这个字符串会作为 HTTP 响应返回给客户端。
...
可以从 [ZooKeeper 官方网站](https://zookeeper.apache.org/) 下载最新版本的 ZooKeeper。
...
```java
package nexus.io.tio.dubbo.demo001.service;
...
```java
package nexus.io.tio.dubbo.demo001.service.impl;

import nexus.io.jfinal.aop.annotation.AService;
import nexus.io.tio.dubbo.demo001.service.HelloService;
...
import nexus.io.tio.dubbo.demo001.service.HiService;
...
```java
package nexus.io.tio.dubbo.demo001.config;
...
import nexus.io.jfinal.dubbo.DubboProvider;
import nexus.io.tio.boot.constatns.TioBootConfigKeys;
import nexus.io.tio.boot.server.TioBootServer;
import nexus.io.tio.dubbo.demo001.service.HelloService;
import nexus.io.tio.dubbo.demo001.service.HiService;
import nexus.io.tio.dubbo.demo001.service.impl.HelloServiceImpl;
import nexus.io.tio.dubbo.demo001.service.impl.HiServiceImpl;
...
```java
package nexus.io.tio.dubbo.demo001;
...
import nexus.io.jfinal.aop.annotation.AConfiguration;
import nexus.io.jfinal.aop.annotation.AInitialization;
import nexus.io.jfinal.dubbo.Dubbo;
import nexus.io.tio.boot.constatns.TioBootConfigKeys;
import nexus.io.tio.boot.server.TioBootServer;
import nexus.io.tio.utils.environment.EnvUtils;
...
- 当在 Dubbo 中调用远程服务时，传递的实际是接口方法的参数，而不是服务对象本身。这些参数在传输过程中被序列化，然后在服务端进行反序列化，最终调用服务实现的方法。
