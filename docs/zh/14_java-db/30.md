...
- 调用 Db.find,query,paginate 等方法时进行写操作
- 其他方法进行写操作
...
- DATABASE_DSN 写库
- DATABASE_DSN_REPLICAS 读库,使用,分隔多个数据库

### 配置类

```java
package com.red.book.config;
...
- 在 `DbConfig` 类的 `@AInitialization` 注解标记的方法 `config()` 中，分别调用 `configMain()` 和 `configReplica()` 方法，完成主库（写库）和从库（读库）的配置。
...
- **启动过程**：在应用启动时，通过 `DbConfig` 类初始化并启动主库和从库的 `ActiveRecordPlugin`，实现读写分离功能。
