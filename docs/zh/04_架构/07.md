...
import com.litongjava.tio.http.common.*;
...
public class MyStaticResourceHandler implements StaticResourceHandler {
    ...
    public HttpResponse handle(String path, HttpRequest request, HttpConfig httpConfig, AbsCache staticResCache) {
        // 检查是否启用静态文件缓存
        boolean enableCache = EnvUtils.getBoolean(ServerConfigKeys.SERVER_RESOURCES_STATIC_FILE_CACHE_ENABLE, false);
        // 获取请求的主机名，并处理冒号
        ...
        // 尝试从缓存中获取文件
        if (enableCache && staticResCache != null) {
            fileCache = (FileCache) staticResCache.get(fileKey);
        }

        if (enableCache && fileCache != null) {
            // 缓存命中，检查是否需要返回304状态码
            ...
            response.setLastModified(HeaderValue.getLastModifiedHeader(lastModified));
            ...
            if (!file.exists()) {
                ...
                if (pageRoot != null) {
                    ...
                    try {
                        ...
                        return Resps.resp404(request, path);
                    }
            ...
            // 缓存文件内容
            if (enableCache && response.isStaticRes() && staticResCache != null) {
                if (response.getBody() != null && response.getStatus() == HttpResponseStatus.C200) {
                    if (content.length <= MAX_CACHE_FILE_SIZE) {
                        ...
                        FileCache newFileCache = new FileCache(content, fileLastModified, contentType, contentEncoding,
                                response.hasGzipped());
                        staticResCache.put(fileKey, newFileCache);
                        log.info("File added to cache: [{}], Size: {} bytes", fileKey, content.length);
                    } else {
                        log.info("File size exceeds cache limit, not cached: [{}], Size: {} bytes", fileKey, content.length);
                    }
                }
            }
        }
