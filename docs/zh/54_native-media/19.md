...
可以通过系统包管理器安装 Noto Sans CJK 字体，下面给出几个常用发行版的安装方法：
...
安装完成后，字体文件一般位于 `/usr/share/fonts/opentype/noto/` 或 `/usr/share/fonts/truetype/noto/` 目录下，可以使用如下命令查看安装的字体文件：
...
在 Arch Linux 上，可以使用 Pacman 来安装：
...
如果的发行版软件源中没有提供对应的包，也可以从 [Google Noto Fonts 的 GitHub 仓库](https://github.com/googlefonts/noto-cjk) 或 [Noto 官网](https://www.google.com/get/noto/#cjk) 下载对应的字体文件（通常是 .ttf 或 .ttc 格式），然后将字体文件复制到系统字体目录（如 `/usr/share/fonts` 或当前用户下的 `~/.fonts`），接着刷新字体缓存：
...
通过以上方法，就可以在 Linux 环境下安装并使用 NotoSansCJK-Regular 字体，从而在视频水印中正确显示中文。
...
通过以上代码，可以实现一个给视频右下角添加中文水印，并将水印视频单独保存的新功能。请根据实际情况调整滤镜参数（例如字体大小、颜色、位置等），并确保提供的字体文件支持中文显示。
...
1. **确认 FFmpeg 是否支持 drawtext：**  
   ...
   如果没有显示 drawtext 滤镜，那么当前使用的 FFmpeg 版本没有启用该滤镜。

2. **使用支持 drawtext 的 FFmpeg 构建版本：**

   - 如果使用的是预编译版 FFmpeg，请选择一个包含 drawtext 支持（通常需要启用 libfreetype）的版本。
   - 或者可以自己编译 FFmpeg。在配置编译参数时，确保启用 libfreetype 支持。例如：
     ```bash
...
3. **在 Windows 平台下使用：**
   ...
   - 如果是 Windows 用户，请检查所使用的 FFmpeg 库或者动态链接库（dll）是否支持 drawtext。部分预编译的 FFmpeg 版本可能为了减小体积而禁用了该滤镜。可以尝试从其他渠道下载一个完整支持 drawtext 的 Windows FFmpeg 构建包。

4. **代码运行时的动态库问题：**
   - 确保在运行 native 库时加载的 FFmpeg 动态库是预期的那一版。如果系统中同时存在多个版本，可能会导致加载了不支持 drawtext 的版本。
