...
JNIEXPORT jobjectArray JNICALL Java_com_litongjava_media_NativeMedia_split(JNIEnv *env, jclass clazz, jstring inputPath, jlong segSize) {
  ...
  // 开始读取输入数据包，并写入当前段（流拷贝）
  while (av_read_frame(ifmt_ctx, pkt) >= 0) {
    // 对 pkt 的时间戳进行重设（以当前流的 time_base 为准）
