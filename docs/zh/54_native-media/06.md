...
该错误原因是：正在将未使用 `-fPIC` 编译的静态 FFmpeg 库链接进共享库，而在 Linux 下，嵌入共享库的所有代码都必须使用 `-fPIC` 编译。
...
- **加载方式：** 如果动态库已经安装在系统默认的库目录中，并且在编译时已正确链接，Linux 动态链接器会自动加载所有依赖项。只需在 Java 中加载主 JNI 库即可：
...
Linux 的动态链接器在加载共享库时，会自动解析并加载该库所依赖的其他共享库（前提是这些库能在默认路径中找到或通过 `LD_LIBRARY_PATH` 指定）。因此，只要将主要的 JNI 库（例如 native_media.so）加载，系统就会自动查找并加载所依赖的 FFmpeg 库，而不必在 Java 中额外调用 `System.loadLibrary`。
