...
```sql
drop table llm_question_recommend;
...
```sql
drop table user_asked_questions;
...
```sql
drop table llm_intent_question;
...
```sql
drop table llm_intent_classification;
...
```
## llm_chat_completion_input
```sql
drop table if exists llm_chat_completion_input;
CREATE TABLE llm_chat_completion_input (
  id BIGINT primary key,
  request jsonb,
  creator VARCHAR(64) DEFAULT '',
  create_time TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
