...
`TioAdminElasticsearchConfig` 不会自动把数据库业务表同步到 Elasticsearch。业务服务需要明确索引字段、写入时机、数据可见性与失败恢复策略：

- 只索引允许公开搜索的字段，避免同步联系方式、密文或私有文件键。
- 在最外层数据库事务提交后同步，避免事务回滚后留下可检索文档；嵌套 Service 调用不应提前写索引。
- 搜索命中后仍由数据库检查审核、下架、删除、过期及作者状态，再进行业务过滤和分页。不能直接相信可能滞后的索引状态。
- 索引故障不应回滚已经提交的业务写入；使用持久化同步任务，或在首版数据规模下使用分批周期校对来修复漏同步。
- 搜索引擎不可用时可回退到数据库查询，但需要明确两种检索的匹配语义差异，并在接口联调信息中标明实际执行路径。
- 不要只取搜索引擎前一页再做权限过滤，否则总数和分页可能错误。应设计完整的候选集过滤或能够正确组合权限条件的索引查询。

验证时使用独立测试索引，覆盖发布、编辑后重审、下架、删除、用户封禁、资源过期、索引失败与恢复。多实例环境下还应验证并发同步的最终一致性。
