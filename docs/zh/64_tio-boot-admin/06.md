...
手动创建 `tio_boot_admin_system_upload_file`，参见[文件信息表](../63_文件存储/01.md)。已建表的应用无需重新建表；文件大小字段为 `size`，单位为字节。
...
public class FileStorageHandler {
  ...
  public HttpResponse upload(HttpRequest request) {
    ...
    String name = ParameterValidator.text(file.getName(), "filename", 64);
    ParameterValidator.require(!name.contains("/") && !name.contains("\\"), "Invalid filename");
    int dot = name.lastIndexOf('.');
    if (dot >= 0) {
      ParameterValidator.require(name.substring(dot + 1).matches("[A-Za-z0-9]{1,10}"), "Invalid file extension");
    }
    String category = ParameterValidator.text(request.getParam("category"), "category", 24, false);
...
文件名和分类长度校验对应示例表中的 `name`、`target_name` 字段容量；HTTP 请求体大小限制应由框架配置控制。
