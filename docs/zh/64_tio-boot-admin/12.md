...
先按 [文件上传](<./06.md>) 配置存储和路由，上传成功后由业务接口验证文件类型、大小和归属，再保存元数据。不能只信任客户端的扩展名、MIME 或大小。
...
验证上传失败不创建文档、跨用户文件 ID 被拒绝、替换失败保留原关联、旧任务不覆盖新版本。逻辑删除与物理文件清理分别定义，避免删除其他记录仍引用的文件。PDF 展示见 [13.md](<./13.md>)。
