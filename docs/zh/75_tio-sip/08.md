...
```java
MediaProcessor mediaProcessor = new RealtimeMediaProcessor();
```
...
1. 现在 `RealtimeModelBridge` 默认按 `24k` 下行处理，所以这个 `RealtimeMediaProcessor` 也是按 `24k -> 8k` 来写的。
   如果后面把桥接层改成别的输出采样率，这里把 `MODEL_OUTPUT_SAMPLE_RATE` 改掉就行。

2. 这个实现是“异步送模型、同步取缓冲”的最小可用版。
   如果后面发现需要更平滑的通话体验，下一步可以加：
...
如果愿意，我可以下一步直接把 `SipServerConfig` 也一并改成可编译的完整版本。
