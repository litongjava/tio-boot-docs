...
5. **验证部署是否成功**  
   ...
   ```
   [root@xxxxx bin]# obclient -h127.0.0.1 -P2883 -uroot -Doceanbase -A
   ...
   ```bash
   [root@xxxxx bin]# ps -ef | grep observer
   ```
...
1. **检查当前运行的集群**  
   ...
   ```bash
   [root@xxxxx bin]# obd cluster list
   ```
...
2. **重启集群**  
   使用 deployname 重启集群：
   ```bash
   [root@xxxxx bin]# obd cluster restart demo
   ```
