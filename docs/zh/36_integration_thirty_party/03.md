...
```properties
MJ_APIKEY_PUBLIC=的公共 API 密钥
MJ_APIKEY_PRIVATE=的私有 API 密钥
```

**注意**：请确保将 `MJ_APIKEY_PUBLIC` 和 `MJ_APIKEY_PRIVATE` 替换为在 Mailjet 控制台获取的实际 API 密钥。
...
```java
package nexus.io.max.blog.utils;
