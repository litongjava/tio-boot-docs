...
创建工具类 `VerifyTokenUtils`，使用 Clerk 后台提供的 JWT 公钥来配置验证选项。代码示例如下：
...
public class VerifyTokenUtils {
  ...
  static {
    // 读取存放 Clerk JWT 公钥的文件
    URL resource = ResourceUtil.getResource("CLERK_JWT_KEY.key");
    if (resource == null) {
...
在 tio-boot 的 `AuthInterceptor` 中，通过解析 HTTP 请求头中的 `authorization` 字段获取 token，并利用 `VerifyTokenUtils.verifyToken` 方法进行验证。验证成功后，提取 token 中的用户标识符（使用 `getSubject()` 方法）并保存到上下文中。
...
import com.litongjava.open.chat.utils.VerifyTokenUtils;
...
public class AuthInterceptor implements HttpRequestInterceptor {
  ...
  public HttpResponse doBeforeHandler(HttpRequest request, RequestLine requestLine, HttpResponse responseFromCache) {
    ...
    if (!test) {
      ...
      try {
        decodedToken = VerifyTokenUtils.verifyToken(idToken);
...
> - 利用 `VerifyTokenUtils.verifyToken` 方法验证 token，验证通过后调用 `getSubject()` 方法获取用户 ID。
