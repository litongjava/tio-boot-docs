...
Clerk 是一款用于处理身份验证和授权的工具包，支持对 JSON Web Token (JWT) 进行验证。通过 Clerk 后台提供的 JWT 公钥，可以对客户端传入的 token 进行解析和验证，从而确保用户身份的真实性。整合 Clerk 后，可以使用标准方法获取 token 中包含的用户信息，例如 `sub` 字段（通常表示用户标识符）。
...
```java
package nexus.io.open.chat.utils;
...
```java
package nexus.io.open.chat.intecerptor;
...
import nexus.io.open.chat.notification.TokenNotificationUtils;
import nexus.io.open.chat.services.NewUserService;
import nexus.io.open.chat.utils.ClerkVerifyTokenUtils;
...
import nexus.io.tio.utils.notification.NotificationTemplate;
...
本文档详细描述了如何在 tio-boot 框架中整合 Clerk 进行 token 验证。通过添加依赖、配置验证工具类以及在拦截器中处理 token 验证流程，可以实现一个高效且安全的认证方案。详细参考 Clerk SDK 的[官方文档](https://github.com/clerk/clerk-sdk-java)获取更多信息和最新更新。
