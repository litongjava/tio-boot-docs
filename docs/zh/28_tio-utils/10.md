...
2. **获取最新的消息更新**  
   ...
   将 `<YourBotToken>` 替换为的 Bot API Token。
...
import nexus.io.tio.utils.http.ResponseVo;
...
```java
package nexus.io.telegram.bot.controller;
import nexus.io.annotation.RequestPath;
import nexus.io.model.http.response.ResponseVo;
import nexus.io.telegram.bot.config.TelegramNotificationConfig;
...
```java
package nexus.io.gpt.translator.services;
...
public class TelegramBotTest {
  ...
  @Test
  public void test() {
    String token = "YOUR_TELEGRAM_BOT_TOKEN"; // 替换为的 Telegram Bot Token
    String webHook = "https://your-domain.com/telegram/webhook"; // 替换为的 Webhook URL
...
1. **替换 Token 和 Webhook URL**  
   将 `YOUR_TELEGRAM_BOT_TOKEN` 替换为从 [BotFather](https://t.me/BotFather) 获取的 Telegram Bot Token，将 `https://your-domain.com/telegram/webhook` 替换为服务器上用于接收 Webhook 请求的 URL。
...
```java
package nexus.io.gpt.translator.controller;
...
public class TelegramWebhookController {
  ...
  public HttpResponse handleTelegramWebhook(HttpRequest request) {
    ...
    if (message != null) {
      ...
      // 回复用户消息
      Telegram.use().sendMessage(chatId.toString(), "发送了: " + text);
    } else {
...
1. **启动服务器**  
   确保的服务器已启动，并且 Webhook URL 可被 Telegram 服务器访问。

2. **设置 Webhook**  
   运行 `TelegramBotTest` 类中的 `test` 方法，设置 Webhook。

3. **发送消息**  
   通过 Telegram 客户端向的 Bot 发送消息，如 "hi"。

4. **观察回复**  
   应收到 Bot 回复的消息 "发送了: hi"。
...
- **SSL 证书**  
  Telegram 要求 Webhook URL 必须使用 HTTPS，并且具备有效的 SSL 证书。确保的服务器配置了正确的 SSL 证书。

- **公网访问**  
  确保的 Webhook URL 对 Telegram 服务器可访问。如果在本地开发，可以使用工具如 [ngrok](https://ngrok.com/) 暴露本地端口。
