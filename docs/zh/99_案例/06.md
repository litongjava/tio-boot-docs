...
```java
package com.litongjava.open.chat.spider.sjsu;
...
public class SJSUClassScheduleProcessor implements PageProcessor {
  ...
  private void getDataList(int i, Element row, Elements columns, String sourceUrl, List<Map<String, Object>> dataList) {
    ...
    dataMap.put("id", SnowflakeIdUtils.id());
...
public class SJSUClassSchedulePipeline implements Pipeline {
  ...
  public void process(ResultItems resultItems, Task task) {
    ...
    for (Map<String, Object> map : dataList) {
      String sql = "select id from rumi_sjsu_class_schedule_2024_fall where class_number=? and term=?";
      Object term = map.get("term");
      Object class_number = map.get("class_number");

      Record record = Db.findFirst(sql, class_number, term);
      if (record != null) {
        Object object = record.get("id");
        updateRecords.add(record.setColumns(map).set("id", object));
      } else {
        saveRecords.add(new Record().setColumns(map));
      }
    ...
    Db.tx(() -> {
      // update
      log.info("update:{}", updateRecords.size());
      Db.batchUpdate("rumi_sjsu_class_schedule_2024_fall", updateRecords, 2000);
      // insert
      log.info("save:{}", saveRecords.size());
      Db.batchSave("rumi_sjsu_class_schedule_2024_fall", saveRecords, 2000);
      return true;
    });
...
public class VectorDatabaseSerchServiceTest2 {
  ...
  public void test() {
    ...
    String classScheduleSql = SqlTpls.get("rumi_sjsu_class_schedule_2024_fall.vector_search");
