...
本章介绍**后台管理前端**及 ApiTable 组件。普通用户使用的 H5、微信小程序可以单独采用 uni-app 等技术，通过业务 API 接入同一后端。两类前端的接口权限不同，不能把后台 Token 或通用表接口直接用于普通用户页面。业务接口与联调约定见 [业务 API 与 H5 / 小程序联调](<./18.md>)。
