...
完成配置后，可以通过 `nexus.io.es.client.Elastic` 类来操作 Elasticsearch。这个类封装了客户端的常见操作，使得与 Elasticsearch 的交互更加简单。

例如，可以通过 `Elastic.getClient()` 来获取 `RestHighLevelClient`，然后执行索引、查询、删除等操作。
