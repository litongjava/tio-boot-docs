...
1. `~/.en` and `~/secrets.txt` (用户目录下的配置文件)
2. `app.properties`（classpath 中存在则加载）
3. `app.properties`（当前工作目录中存在则加载）
4. `.env`（classpath 中存在则加载）
5. `.env`（当前工作目录中存在则加载）
6. `secrets.txt`（当前工作目录）
7. `my.txt`（当前工作目录，优先级最高）
...
```java
String env = EnvUtils.getAppEnv(); // 或 EnvUtils.env()
```
