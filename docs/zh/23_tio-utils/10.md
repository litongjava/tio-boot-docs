...
1. **替换 Token 和 Webhook URL**：将 `YOUR_TELEGRAM_BOT_TOKEN` 替换为你从 [BotFather](https://t.me/BotFather) 获取的 Telegram Bot Token，将 `https://your-domain.com/telegram/webhook` 替换为你服务器上用于接收 Webhook 请求的 URL。
...
1. **接收请求**：`handleTelegramWebhook` 方法接收来自 Telegram 的 HTTP POST 请求。
...
- **公网访问**：确保你的 Webhook URL 对 Telegram 服务器可访问。如果你在本地开发，可以使用工具如 [ngrok](https://ngrok.com/) 暴露本地端口。
...
通过以上步骤和说明，开发者可以快速上手使用 `tio-utils` 的 `TelegramBot` 实现 Telegram 消息的接收与回复功能。如有更多问题，建议参考 [Telegram Bot API 官方文档](https://core.telegram.org/bots/api) 或联系相关技术支持。
