...
1. [内部 Cache 负责存储的信息](#内部-cache-负责存储的信息)
2. [使用 Redis 作为内部 Cache](#使用-redis-作为内部-cache)
   - [添加依赖](#添加依赖)
   - [添加配置](#添加配置)
   - [添加配置类](#添加配置类)
   - [启动类](#启动类)
...
3. [关键配置说明](#关键配置说明)
   - [`server.cache.store=redis`](#servercachestoreredis)
   - [`@BeforeStartConfiguration`](#beforestartconfiguration-注解)
   - [`RedisMapCacheFactory`](#redismapcachefactory)
4. [总结](#总结)
...
public class BeforeTioStartConfig {
  ...
  public void config() {
    ...
    // 将 Jedis 连接池设置到 RedisMapCacheFactory
...
- `RedisMapCacheFactory.INSTANCE.setJedisPool(jedisPool)`：将创建的 Jedis 连接池注入到 `RedisMapCacheFactory` 中，使其能够使用 Redis 进行缓存操作。
- `TioBootServer.me().addDestroyMethod(jedisPool::close)`：注册应用关闭时的销毁方法，确保 Redis 连接池被正确关闭，释放资源。
...
`RedisMapCacheFactory` 是 Tio-boot 提供的一个缓存工厂类，负责管理 Redis 作为缓存存储的具体实现。通过将 `JedisPool` 注入到 `RedisMapCacheFactory`，它可以使用 Redis 进行缓存的读写操作。
...
1. **初始化 Redis 连接池**：在配置类中创建 `JedisPool` 并注入到 `RedisMapCacheFactory`。
...
- **监控与优化**：定期监控 Redis 的性能，调整连接池和缓存策略参数，以确保最佳性能。
- **安全配置**：在生产环境中，确保 Redis 配置了强密码，并通过防火墙等手段限制访问，保障数据安全。
- **高可用部署**：考虑使用 Redis 集群或主从架构，提升系统的可用性和容错能力。

通过合理配置和优化，Redis 将为您的 Tio-boot 应用提供强大的缓存支持，显著提升系统的响应速度和处理能力。
