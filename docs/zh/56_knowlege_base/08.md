...
- **方法 `pageDocument`**：
  - **注解 `@Get("/{id}/document/{pageNo}/{pageSize}")`**：定义 GET 请求路径。
  - **参数**：
    - `id`：数据集 ID。
    - `pageNo`：当前页码。
    - `pageSize`：每页文档数量。
...
public class MaxKbDocumentService {
  ...
  public ResultVo page(Long userId, Long datasetId, Integer pageNo, Integer pageSize) {
    ...
    // 如果用户不是管理员（假设 userId 为 1 为管理员），则添加用户 ID 过滤
    if (userId != 1) {
  ...
  public ResultVo get(Long userId, Long datasetId, Long documentId) {
    ...
    // 如果用户是管理员，则仅根据数据集 ID 过滤
    if (userId == 1) {
      tableInput.set("dataset_id", datasetId);
    } else {
      // 否则，根据用户 ID 和数据集 ID 过滤
      tableInput.set("user_id", userId)
...
- **方法 `page`**：
  ...
  - **功能**：
    ...
    5. 从 `TableResult` 中提取 `Page<Record>` 对象，获取总行数和记录列表。
...
- **方法 `get`**：
  ...
  - **功能**：
    ...
    3. 调用 `ApiTable.get` 方法获取单条记录 (`Record`)。
    4. 将 `Record` 转换为键值对 (`Kv`)。
    5. 返回成功的 `ResultVo` 对象，包含文档详细信息。
...
通过上述接口和代码实现，我们成功地为系统添加了文档管理功能。该功能允许用户根据数据集管理和检索文档，支持分页查询和详细信息查看。未来，可以进一步扩展此功能，如添加文档上传、编辑、删除等操作，以满足更全面的文档管理需求。
