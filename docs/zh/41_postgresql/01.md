...
- `-e POSTGRES_PASSWORD=123456`：设置 PostgreSQL 的超级用户密码为 `123456`。
- `litongjava/postgres:16.3`：指定使用的 PostgreSQL 镜像及其版本。
...
- `mkdir -p /docker_data/postgres/master`：在宿主机上创建一个目录用于存放 PostgreSQL 数据。
- `docker cp postgresql:/var/lib/postgresql /docker_data/postgres/master`：将容器内的 PostgreSQL 数据目录复制到宿主机的备份目录中。
...
- `docker stop postgresql`：停止名为 `postgresql` 的容器。
- `docker rm postgresql`：移除已停止的容器。
...
-e TZ=Asia/Shanghai \
-v /docker_data/postgres/master:/var/lib/postgresql \
litongjava/postgres:16.3
```

- `--name pgmaster`：为新的容器命名为 `pgmaster`。
- `--restart=always`：设置容器自动重启策略，确保容器在系统重启后自动启动。
- `-e PGDATA=/var/lib/postgresql/data`：指定 PostgreSQL 数据目录的位置。
- `-e POSTGRES_PASSWORD=123456`：设置 PostgreSQL 的超级用户密码。
- `-e TZ=Asia/Shanghai`：设置容器的时区为上海时间。
- `-v /docker_data/postgres/master:/var/lib/postgresql`：将宿主机的 `/docker_data/postgres/master` 目录挂载到容器内的 `/var/lib/postgresql`，实现数据持久化。
- `litongjava/postgres:16.3`：指定使用的 PostgreSQL 镜像及其版本。
...
您应能看到名为 `pgmaster` 的容器正在运行，并且端口 `5432` 已正确映射。
