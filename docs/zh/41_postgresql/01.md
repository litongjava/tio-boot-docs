...
- `docker cp postgresql:/var/lib/postgresql/data /docker_data/postgres/master/`：将容器内的 PostgreSQL 数据目录复制到宿主机的备份目录中。
...
- `-v /docker_data/postgres/master/data:/var/lib/postgresql/data`：将宿主机的 `/docker_data/postgres/master/data` 目录挂载到容器内的 `/var/lib/postgresql/data`，实现数据持久化。
- `litongjava/postgres:16.3`：指定使用的 PostgreSQL 镜像及其版本。
