...
tio-utils 内置了邮箱工具类 nexus.io.tio.utils.email.EMailUtils 用于支持发送邮件
...
- 如果使用这个版本，的 Maven 配置依赖的是较旧的邮件 API 版本。
...
如果切换到 `com.sun.mail:javax.mail:1.6.2` 依赖，代码可以基本保持不变，因为 API 本身没有大的改变。只需要在 `pom.xml` 文件中替换依赖接口
...
如果需要更多的新特性、安全性，建议使用 `com.sun.mail:javax.mail:1.6.2`。
...
EMailUtils.send("litongjava@qq.com", "验证码", "欢迎登录XXX系统,的验证码是 1234");
```
...
```java
import nexus.io.tio.utils.email.EMailUtils;
import nexus.io.tio.utils.email.GMailFactory;
...
- 1. 实现类 nexus.io.tio.utils.email.EMail 重写 send 方法
- 2.实现类 nexus.io.tio.utils.email.IEMailFactory 重写 getMail 方法 返回 Mail
- 3. 调用 nexus.io.tio.utils.email.EMailUtils.setEMailFactory(IEMailFactory) 设置实现类
