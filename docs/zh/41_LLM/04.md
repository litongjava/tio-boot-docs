...
这里创建了两种表,一张存储查看意图分类,一张存储意图例句

### 初始数据

假设将意图分为 3 类,添加 3 个分类分别是 course_registration_and_enrollment,financial_aid_and_tuition,campus_services_and_resources,并添加 additional_info
...
调用 OpenAI 的接口,将例句向量化方便后续的的 余弦相似度计算,向量化的示例代码如下
