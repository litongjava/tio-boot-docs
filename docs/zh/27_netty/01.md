...
如果需要自定义线程池,请参考下面的配置,ThreadUtils 是需要手动编写的工具类,内部实现可以参考 TioThreadUtils
...
```java
package nexus.io.tio.web.hello.nettyserver;
...
```java
package nexus.io.tio.boot.hello.nettyserver;
...
import nexus.io.tio.web.hello.nettyserver.MyChannelInitializer;
import nexus.io.tio.web.hello.nettyserver.NettyServerBootstrap;
...
- `initChannel(SocketChannel socketChannel)`: 在这个方法中，可以添加各种处理器到这个通道的流水线（`ChannelPipeline`），这里添加了 `MyInbloudHandler` 实例。
