...
```java
import nexus.io.tio.boot.grovvy.GroovyScriptManager;
import nexus.io.tio.boot.server.TioBootServer;
import nexus.io.tio.http.server.router.HttpRequestRouter;

public class GroovyHandlerConfig {
...
> **说明**  
> 通过配置 `HttpRequestRouter`，将 `/hi` 路由与脚本执行逻辑进行绑定。当 HTTP 请求到达 `/hi` 时，系统会加载并执行 `web_hello.groovy` 脚本，并将脚本返回的结果作为 HTTP 响应发送回客户端。
...
```java
package com.jobright.study.voice.agent.handler;
...
public class GroovyHandler implements HttpRequestHandler {
...
```java
r.add("/groovy", new GroovyHandler());
```
...
def appEnv = EnvUtils.getAppEnv()


return Kv.by("app.env",appEnv)
```
