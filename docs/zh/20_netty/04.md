...
```java
package nexus.io.im.netty.handler;
...
import nexus.io.im.netty.protobuf.ChatPacketOuter;
import nexus.io.im.netty.protobuf.ChatPacketOuter.ChatPacket;
import nexus.io.im.netty.utils.MessageUtils;
...
通过以上步骤，可以将客户端和服务器之间的消息从 JSON 转换为二进制格式，进一步提高传输效率并减少数据量。这种方法特别适用于高性能应用场景，如实时通信或大规模数据交换。
