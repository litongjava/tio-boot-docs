...
1. [Maven 依赖配置](#maven-依赖配置)
2. [应用配置文件设置](#应用配置文件设置)
3. [MailJet 发送邮件示例](#mailjet-发送邮件示例)
4. [封装工具类](#封装工具类)
5. [测试工具类](#测试工具类)
6. [整合 Enjoy 发送邀请邮件](#整合-enjoy-发送邀请邮件)
   - [邮件模板](#邮件模板)
   - [EmailService 服务类](#emailservice-服务类)
   - [工具类扩展](#工具类扩展)
7. [发送后的响应示例](#发送后的响应示例)
...
public class MailJetUtils {
    ...
    public static MailjetResponse sendText(String toName, String toEmail, String subject, String text) {
        ...
        JSONArray to = new JSONArray()
                .put(new JSONObject().put("Email", toEmail).put("Name", toName));
...
import com.litongjava.open.chat.utils.MailJetUtils;
...
public class EmailService {
    ...
    public RespBodyVo sendInviteEmail(String origin, long[] studentIds) {
        ...
        for (long id : studentIds) {
            ...
            // 查询学生邮箱
            String sql = String.format("SELECT email FROM %s WHERE id=?", TableNames.max_kb_student);
