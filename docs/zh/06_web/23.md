...
- tio-boot 内置了线程池工具类 nexus.io.tio.utils.thread.TioThreadUtils,并且 tio-boot 在关闭是 会自动释放线程资源
  示例
...
1. **线程池管理**：
   ...
   - 如果使用 `runAsync(Runnable runnable, Executor executor)`，任务会由提供的自定义 `Executor` 执行，这给了更大的灵活性来控制线程的使用。
...
4. **任务的生命周期**：
   ...
   - 可以在 `CompletableFuture` 完成后设置回调函数，如 `.thenRun(...)`，这使得异步任务的结果可以在完成后被处理。
...
```
import nexus.io.apps.asrgpt.services.AsrSubmitService;
import nexus.io.apps.asrgpt.validator.AsrSubmitValidator;
