...
整合 SSE 到 tio-boot 框架中可以让的应用具备实时数据推送的能力。以下是在 tio-boot 框架中创建一个简单的 SSE 应用的步骤和代码示例：
...
```java
package nexus.io.ai.chat.AController;

import nexus.io.tio.core.Tio;
import nexus.io.tio.http.common.HttpRequest;
import nexus.io.tio.http.common.HttpResponse;
import nexus.io.annotation.RequestPath;
import nexus.io.tio.http.server.sse.SsePacket;
...
要测试的 SSE 服务，可以使用 curl 命令访问 SSE 路径：
...
```java
package nexus.io.manim.handler;
...
通过上述步骤，可以在 tio-boot 框架中成功整合 SSE，从而使的应用能够实时地向客户端推送数据。这种方法的优点在于其简单性和低延迟，非常适用于需要服务器实时更新的场景。
