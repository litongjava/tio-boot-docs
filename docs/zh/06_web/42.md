...
**示例**：假设使用了 `swagger-ui` 的 WebJar 依赖，`swagger-ui` 的静态文件会放在 `META-INF/resources/webjars/swagger-ui` 下。在 Web 服务器中，可以通过 `/webjars/swagger-ui` 访问它们。
...
import nexus.io.tio.web.hello.handler.HelloHandler;
