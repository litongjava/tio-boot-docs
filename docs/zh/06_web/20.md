...
#### 2. HttpRequestInterceptor（通用业务拦截器）
...
```java
package nexus.io.demo.interceptor;
...
为此，Tio-Boot 提供了 **UserTokenInterceptor** 内置拦截器，它们配合使用可以满足上述需求。
...
通过 **UserTokenInterceptor** 的配合使用，Tio-Boot 提供了一套灵活、安全的 **Token 验证与用户上下文管理机制**，其优势包括：
