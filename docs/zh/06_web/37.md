...
本文档将帮助快速在项目中启用并定制 AES/CBC/PKCS5Padding 算法的 HTTP 加密。
...
实现 `nexus.io.tio.boot.encrypt.TioEncryptor` 接口，示例使用 AES-128/CBC/PKCS5Padding，并在密文前拼接随机 IV。
...
如果直接使用低级别的 Handler（而非 Controller），则需在生成响应体时自行调用加密器：
...
* **Json 序列化**：可以替换为任意 JSON 工具，关键是拿到 `byte[]`。
...
1. 从响应体中取出前 16 字节 IV；
2. 使用与服务端一致的 AES-128 Key 与 IV 对剩余字节进行解密；
...
通过上述步骤，即可在 tio-boot 项目中快速启用并定制 HTTP 返回数据的 AES 加密，满足对传输安全的需求。
