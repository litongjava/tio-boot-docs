...
响应可以使用 Map 自定义结构，推荐使用统一响应对象 `RespBodyVo`。当前 `nexus.io` 版本的类位于 `nexus.io.model.body.RespBodyVo`，由 java-model 提供，tio-boot 项目可使用。
...
除了基本的成功或失败状态，`RespBodyVo` 还允许设置额外的属性，如消息(`msg`)、业务编码(`code`)和业务数据(`data`)。
