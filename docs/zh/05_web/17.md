...
public class LoginValidator {
  public RespBodyVo validateLogin(String username, String password, String verificationCode) {
    ...
    if (StrKit.isBlank(username)) {
      retval = RespBodyVo.fail("The username or password cannot be empty");
      return retval;
    }
...
1. `LoginValidator` 类：
   ...
   - 首先检查 `username` 是否为空，如果为空，则返回一个包含错误消息的 `RespBodyVo` 对象。
   - 方法还可以进一步验证其他字段，如果所有验证都通过，则返回 `null`；如果任何验证不通过，则返回包含错误信息的 `RespBodyVo`。

2. `LoginController` 类：
   - 通过 `@RequestPath("/auth")` 注解映射到 `/auth` 路径。
   - `login` 方法接收登录参数并使用 `Aop.get(LoginValidator.class).validateLogin` 调用验证器进行验证。
   - 如果验证结果非空（即有错误），方法返回这个验证结果；如果验证通过（验证结果为空），则返回 `RespBodyVo.ok()` 表示登录成功。
