...
public class GlobalExceptionHandler implements TioBootExceptionHandler {
  ...
  public RespVo handler(HttpRequest request, Throwable e) {
    ...
    log.error("请求URL: {}, 请求头: {}, 请求体: {}, 异常堆栈: {}", requestURL.toString(), headers, bodyString, stackTrace);
...
- `@AInitialization` 注解标记的方法 `config` 会在服务器启动时执行。
  ...
  public RespVo handler(HttpRequest request, Throwable e) {
    ...
    if (!EnvUtils.isDev()) {
      TioThreadUtils.submit(() -> {
        ...
        // 保存到数据库
        Db.save("tio_boot_admin_system_exception", Record.by("id", id).set("exception", stackTrace));
        // 发送告警通知（如微信、邮件等）
