...
本文档将指导在 Docker 环境下部署 **Max-Search** 项目的后端服务，并初始化数据库所需的环境。同时，还将介绍前端的运行方式。
[[toc]]
...
可以参阅官方文档或 [tio-boot 的 PostgreSQL 教程](https://www.tio-boot.com/zh/41_postgresql/01.html) 进行安装和基本配置。
...
4. **导入初始化数据**  
   ...
   ```
   其中 `<username>` 替换为的 PostgreSQL 用户名。

---

## 3. 部署后端

在完成数据库配置后，可以使用 Docker 部署 **Max-Search** 后端容器。以下是基本的启动命令示例：
...
> **说明：**
>
> 1. `--net=host` 参数表示容器将使用主机网络。也可以根据需求修改网络模式。
...
1. **编辑配置文件**  
   ...
   其中 `NEXT_PUBLIC_API_URL` 为后端服务地址，如在本地测试，可将其改为 `http://localhost/api` 或其他真实后端地址。
...
至此，已完成 **Max-Search** 后端在 Docker 环境中的部署，并配置好了 PostgreSQL 数据库。前端目前不支持 Docker，可在本地环境或服务器上通过 Yarn/NPM 启动。若有更多疑问或遇到问题，可在项目的 [GitHub Issues](https://github.com/litongjava/max-search/issues) 中进行反馈，或查阅 [tio-boot.com](https://www.tio-boot.com/) 上的相关文档获取帮助。

感谢对 **Max-Search** 项目的关注与支持！祝部署顺利、使用愉快。
