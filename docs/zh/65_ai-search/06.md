...
import nexus.io.perplexica.services.SearxngSearchService;
...
可以使用 `curl` 命令直接请求 Jina Reader API 读取页面内容：
...
import nexus.io.jian.reader.JinaReaderClient;
import nexus.io.tio.utils.environment.EnvUtils;
...
通过本文档，可以快速了解系统各模块的设计理念与实现细节，同时根据实际场景灵活选择合适的网页内容读取方案。
