...
1. **添加依赖**：
   在的项目中添加 Elasticsearch 的依赖。使用 Maven 或 Gradle 来管理依赖项。以下是 Maven 依赖：
   注意版本 需要和 服务端的版本相匹配
...
3. **使用 Elasticsearch 客户端进行操作**：
   可以使用 `RestHighLevelClient` 进行各种操作，如索引文档、搜索文档等。下面是一个示例，展示如何索引和搜索文档：

```java
package nexus.io.tio.web.hello.example;
...
import nexus.io.tio.web.hello.config.ElasticsearchConfig;
...
通过以上步骤，就可以在一个普通的 Java 工程中成功整合 Elasticsearch 并进行基本的操作了。
