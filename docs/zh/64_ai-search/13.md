...
- GeminiSseCallback 解析 Gemini 模型返回的 SSE 数据，读取每一行数据，并解析其中的 JSON 数据，提取生成的文本片段。
...
- **回调处理**：通过 DeepSeekSseCallback 和 GeminiSseCallback 两个回调类实现对第三方响应的实时解析与客户端通知。
