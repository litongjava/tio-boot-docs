...
Google Custom Search JSON API 允许开发者使用 HTTP GET 请求，获取指定搜索范围内的搜索结果。利用该 API，可以将搜索目标限定在特定网站（如 `sjsu.edu`），并通过编程方式处理返回的 JSON 格式数据。需要注意的是，使用该 API 前必须具备以下两个必要条件：
...
4. 创建完成后，系统会为生成一个搜索引擎 ID，即 `cx` 值，例如：
   ```
...
- **API 必需参数**：Custom Search JSON API 要求请求中携带 `cx` 参数，此值来源于预先创建的 CSE。
- **搜索范围控制**：通过在 CSE 中配置包含的网站或在查询关键词中使用 `site:sjsu.edu`，可以限定搜索范围。
...
确保已经获得了一个 API Key，并在 Google Cloud API Console 中启用了对 Custom Search JSON API 的访问权限。可以在控制台中查看该 API Key 的配额与权限配置。
...
如果的 CSE 已经限定了搜索域为 `sjsu.edu`，只需在 `q` 中填写关键词即可。例如，搜索 “registration”：
...
**回答**：是的。Custom Search JSON API 需要 `cx` 参数，该值来源于预先创建并配置的 CSE。如果没有创建 CSE，将无法获得合法的 `cx`，进而无法正常调用 API。
...
- **API Key**：确保拥有一个已启用 Custom Search JSON API 权限的 API Key。
...
通过以上步骤，就能利用现有的 API Key 和已配置的 CSE，针对特定网站（如 `sjsu.edu`）实现搜索功能，并获取 JSON 格式的搜索结果。

---

这份文档详细说明了使用 Google Custom Search JSON API 的全过程，确保在开发过程中可以顺利集成并调用该 API。
