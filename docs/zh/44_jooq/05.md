...
跳过jooq生成
```bash
mvn clean install -DskipTests -Dgpg.skip -Djooq.codegen.skip=true
```
...
后续也可以进一步使用 Converter，把 jsonb 转成 `Map<String,Object>`（通常需要自定义 converter 类）。
...
加入 jOOQ Codegen 后，的 tio-boot + jOOQ 体系会从“可用”提升为“强类型工程化”：
