...
```java
package com.litongjava.file.dao;
...
public class SystemUploadFileDao {
  ...
  public Record getFileBasicInfoByMd5(String bucketName, String md5) {
    String sql = "select id,filename,bucket_name,target_name from tio_boot_admin_system_upload_file where bucket_name=? and md5=? and deleted=0";
