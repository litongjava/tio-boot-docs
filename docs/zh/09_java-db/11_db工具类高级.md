...
1. **`sql`**：指的是`INSERT INTO`或`UPDATE`语句。动态参数使用问号（`?`）作为占位符，例如：
   ...
   上述 SQL 语句中需要插入`name`和`cash`两个字段，`values`中的两个问号将从后续的`modelOrRecordList`中获取对应的值。

2. **`columns`**：指的是前面 SQL 语句中问号占位符对应的参数名称。Java-db 在填充字段值时，会根据这个名称从`modelOrRecordList`中提取数据。

3. **`modelOrRecordList`**：指的是`List<User>`或`List<Record>`等类型的列表，包含需要批量操作的数据。
...
```java
int[] batchSave(String tableName, List<? extends Record> recordList, int batchSize)
```

### 批量更新

```java
int[] batchUpdate(String tableName, List<? extends Record> recordList, int batchSize)
```
...
即使`"t_maven_dependency"`表中包含多个字段，`batchSave`方法只会插入`Record`中存在的字段，从而提高了操作的灵活性和效率。
...
import com.litongjava.Java-db.plugin.activerecord.Db;
import com.litongjava.Java-db.plugin.activerecord.Record;
...
3. **方法 `batchUpdateByIds(DbPro dbPro, String tableName, Kv kv)`**
   ...
   - **构造更新记录**：
     - 创建一个空的`Record`列表。
     - 遍历`ids`，对每一个 ID，创建一个`Record`，将除`ids`外的`kv`键值对设置为`Record`的列，并设置主键字段为对应的 ID。
   - **执行批量更新**：
...
## 独立使用 ActiveRecord 进行数据批量保存

以下示例展示了如何独立使用 Java-db 的 ActiveRecord Java-db 进行批量保存操作。
...
import com.Java-db.plugin.activerecord.ActiveRecordPlugin;
import com.Java-db.plugin.activerecord.Db;
import com.Java-db.plugin.activerecord.Record;
...
public class AudioToTextAndSaveDb {
    public static void main(String[] args) throws IOException {
        ...
        for (Recognized recognized : recognizedList) {
            Record record = new Record();
            record.set("pcm_file", recognized.getPcmFile());
            record.set("text", recognized.getText());
...
3. **准备批量插入数据**：将`Recognized`对象转换为`Record`对象，添加到`insertList`中。
4. **配置数据库连接**：使用 Druid Java-db 和 ActiveRecord Java-db 配置数据库连接。
5. **批量保存数据**：调用`Db.batchSave`方法将数据批量保存到`recognized`表中。

**注意**：代码中还有许多优化空间，例如将数据进行拆分、分线程处理等。

## 独立使用 ActiveRecord 进行数据的批量插入

以下示例展示了如何使用 Java-db 的 ActiveRecord Java-db 从源数据库查询水位数据，处理后插入到目标数据库。
...
public class WaterLevelImportService {
    ...
    public void fromDatasource1ToDatasource2() {
        ...
        List<Record> sourceRecords = Db.use(ds1).find(sqlSource);
        log.info("水位总条数:{}", sourceRecords.size());
        ...
        for (Record r : sourceRecords) {
            ...
            Record countRecord = Db.use(ds2).findFirst(sqlCheck, r.getStr("site_name"), timeAMString);

            if (countRecord.getInt("COUNT(*)") == 0) {
                String randomId = UUIDUtils.random();
                Record insertRecord = new Record();
                insertRecord.set("id", randomId);
            ...
            countRecord = Db.use(ds2).findFirst(sqlCheck, r.getStr("site_name"), timePMString);
...
1. **启动数据源**：通过 Druid 和 ActiveRecord Java-db 分别配置源数据库（`datasource1`）和目标数据库（`datasource2`），并启动连接。
...
以下示例展示了如何从网络获取数据，转换为`JSONObject`，再转换为 Java 对象，最终转为`Record`对象并批量保存到数据库中。
...
import top.ppnt.chaofu.job.db.test.ActiveRecordInit;
...
public class GetHttpData {
    ...
    public void getHttpData() throws IOException {
        ...
        // 3.保存到数据库
        List<Record> insertList = new ArrayList<Record>(size);
        for (int i = 0; i < size; i++) {
...
1. **初始化 ActiveRecord**：在测试开始前，通过`ActiveRecordInit.init()`方法初始化 ActiveRecord Java-db。
2. **获取 HTTP 数据**：
...
5. **转换为`Record`对象**：
   - 将 Java 对象的属性设置到`Record`对象中，并添加到插入列表`insertList`中。
6. **批量保存数据**：
