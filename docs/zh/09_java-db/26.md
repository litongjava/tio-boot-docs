...
java-db 的 `Db` 工具类内置了读写分离功能。未处于默认数据源事务中时，自动读路由选择读数据源；默认数据源事务中，自动读路由固定使用主库的事务连接，以保证读取本事务写入的数据及行锁有效。写操作使用写数据源。
...
import nexus.io.db.activerecord.ReplicaActiveRecordPlugin;
...
`Db.tx`、`Db.txResult` 内的自动读选择（包括 Db.useRead 和 DbKit.getReadConfig）使用主事务连接，退出事务后恢复读副本选择。这不是跨库分布式事务。

显式调用 `Db.use(name)`、`Db.useReplica(name)` 或按索引指定副本是调用者的选择，不会被自动改成另一个数据源。命名数据源事务中的全部操作需使用相同的 DbPro；不要在其中混入默认 Db 查询。
