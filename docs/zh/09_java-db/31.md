...
import nexus.io.db.activerecord.stat.LiteSqlStatementStat;
...
运行测试类后，会看到 SQL 查询的结果以及 SQL 统计信息，例如：
...
通过这些配置，可以轻松记录和查看 SQL 的执行情况，为后续的性能优化和问题排查提供有力支持。
