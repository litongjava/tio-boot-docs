...
使用 java-db 库时，单数据源业务直接调用静态 `Db`，复用 ActiveRecord 已初始化的默认配置，无需持有 DbPro 实例；JSONB 显式解析、命名数据源、事务边界和测试隔离见 [PostgreSQL 业务实践](<./32.md>)。
...
如果编写了 sql 语句,需要进行插入,更新获取删除操作,请使用 updateBySql 方法,默认的 update 方法不支持传入 sql
使用示例
...
- `Db.txResult(...)`：事务提交后返回业务数据，无需手写结果容器。
- `Db.findFirstWithJsonField(...)`：读取第一条记录并解析明确指定的 JSON 列；无记录返回 null。

两者都提供对应的 DbPro 入口。参数绑定、回滚和业务层职责见 [PostgreSQL 业务实践](./32.md)。
