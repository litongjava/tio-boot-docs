...
`ActiveRecord` 模块的 `nexus.io.db.activerecord.generator` 包下提供了 `Generator` 工具类，能够自动生成 `Model`、`BaseModel`、`MappingKit`、`DataDictionary` 四类文件。生成后的 `Model` 类将与 Java Bean 结合，立即拥有 getter 和 setter 方法，遵循传统的 Java Bean 规范，开发过程中无需手动记忆字段名。
...
```java
package nexus.io.diamond.boradcast;
...
public class JavaDbGenerator {
  public static String modelPackageName = "nexus.io.common.model";
...
```java
package nexus.io.common.model;

import nexus.io.common.model.base.BaseTioBootAdminSystemConstantsConfig;
...
```java
package nexus.io.common.model.base;
...
```java
package com.example.auth.app.db.model;

import com.example.auth.app.db.model.base.BaseTioBootAdminSystemConstantsConfig;
...
import nexus.io.common.model._MappingKit;
...
import com.example.auth.app.db.model.TioBootAdminSystemConstantsConfig;
