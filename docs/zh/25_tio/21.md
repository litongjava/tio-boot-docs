...
`GroupStat` 是 `ChannelStat` 的集合，用于统计所有 TCP 通道的流量数据和状态。它有两个子类：`ServerGroupStat` 和 `ClientGroupStat`。当 t-io 用作 TCP 服务器时，将获取 `ServerGroupStat`，反之则获取 `ClientGroupStat`。
