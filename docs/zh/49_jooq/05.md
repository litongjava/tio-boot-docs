...
示例代码以 jOOQ 为主，尽量使用 **生成代码（强类型）**；若当前未启用 codegen，也会给出可替代写法思路。
...
如果启用了 jOOQ codegen，字段通常会有 `USER_PROFILE.PROFILE`（类型可能是 `JSONB`）。
