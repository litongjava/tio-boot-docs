...
public class JooqBootConfig implements BootConfiguration{
  ...
  public void config() throws Exception {
    AopInterceptorManager.me().registerInterceptor(ATransactional.class, TransactionInterceptor.class);
  }
}
```
...
@Slf4j
public class QaTestConfig implements BootConfiguration {

  @Override
  public void config() throws Exception {
    //需要提前添加
    AopInterceptorManager.me().registerInterceptor(Transactional.class, TransactionInterceptor.class);

    String jdbcUrl = EnvUtils.get(ServerConfigKeys.JDBC_URL);
    String username = EnvUtils.getStr(ServerConfigKeys.JDBC_USER);
    String password = EnvUtils.getStr(ServerConfigKeys.JDBC_PSWD);

    // 1) 连接工厂配置（JDBC URL / 驱动 / 账号密码）
    AgroalConnectionFactoryConfigurationSupplier connectionFactory = new AgroalConnectionFactoryConfigurationSupplier()
        // PostgreSQL 驱动类（按你的数据库替换）
        .connectionProviderClass(org.postgresql.Driver.class).jdbcUrl(jdbcUrl)
        .principal(new io.agroal.api.security.NamePrincipal(username))
        .credential(new io.agroal.api.security.SimplePassword(password));

    // 2) 连接池配置（按需调优）
    AgroalConnectionPoolConfigurationSupplier poolConfig = new AgroalConnectionPoolConfigurationSupplier()
        .connectionFactoryConfiguration(connectionFactory).minSize(0).maxSize(20)
        // 获取连接超时
        .acquisitionTimeout(Duration.ofSeconds(30))
        // 可选：连接最大生命周期
        .maxLifetime(Duration.ofMinutes(30));

    // 3) DataSource 配置 + 创建数据源
    AgroalDataSourceConfigurationSupplier dsConfig = new AgroalDataSourceConfigurationSupplier()
        .connectionPoolConfiguration(poolConfig)
        // 可选：开启 metrics（你如果不用就删掉）
        .metricsEnabled(true);

    AgroalDataSource agroalDataSource = AgroalDataSource.from(dsConfig);

    // 4) jOOQ DSLContext
    DSLContext dslContext = DSL.using((DataSource) agroalDataSource, SQLDialect.POSTGRES);

    // 5) 注册核心对象
    AopManager.me().addSingletonObject(AgroalDataSource.class, agroalDataSource);
    AopManager.me().addSingletonObject(DSLContext.class, dslContext);

    // 6) 注册事务管理器（你的 TransactionManager 如果只依赖 DataSource，直接传 agroalDataSource 即可）
    TransactionManager txManager = new TransactionManager(agroalDataSource, SQLDialect.POSTGRES);
    AopManager.me().addSingletonObject(TransactionManager.class, txManager);
    Aop.addFetchBeanAnnotation(Inject.class);

    log.info("jOOQ + TransactionManager initialized (Agroal)");
