...
import nexus.io.gpt.translator.predicate.UserPredicate;
import nexus.io.gpt.translator.tgfunc.TranslateFunction;
...
```java
package nexus.io.gpt.translator.tgfunc;
...
2. **`flatMap` 的作用：**
   ...
   - **链式异步操作：** 当有一个依赖于前一个异步操作结果的后续异步操作时，使用 `flatMap` 可以将它们串联起来，确保前一个操作完成后再执行下一个操作。
