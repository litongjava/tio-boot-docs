...
* **控制层（Controller / HTTP 接口）**：接收来自前端的 HTTP 请求，并通过 Tio 框架将请求委托给内置的服务类。
...
* **数据层（ActiveRecord 与数据库）**：使用 JFinal ActiveRecord 模式对 PostgreSQL（或其他关系型数据库）表进行插入、更新、查询，保存视频生成记录、LLM 使用日志、错误修复内容、缓存元数据等。
