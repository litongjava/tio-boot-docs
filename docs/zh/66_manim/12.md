...
- 第一个场景的视频渲染完成后，前端即可播放；
- 后续场景生成的代码经过分割生成视频切片，然后追加到前面的 m3u8 文件中；
- 如果某个场景的代码运行出错，则通过交互流程，让大模型自动修复代码，再继续生成下一个场景。

下面详细介绍整个交互流程、各场景提示以及代码示例。

---

## 1. 背景概述

由于一次性生成完整的视频渲染代码可能会存在生成时间长、运行错误率高等问题，因此采用分场景生成策略。  
具体流程如下：

1. 调用大模型生成第 1 个场景的 Python 脚本代码。
2. 将生成的代码发送到远程服务器（java-linux）执行，由 java-linux 自动处理视频文件和流式播放支持。
3. 如果运行出错，再由大模型给出修复后的代码并生成下一个场景。
4. 每个场景渲染后的 m3u8 文件都可以供前端实时播放。
5. 最后将所有场景的视频合并得到完整视频文件。
...
Please ensure that the generated code for the next scene is synchronized with the previous scene's code in terms of interface style, coordinate system, and elements.If there is no next scene, output done.
```
