...
```
Please only output the runnable Python script code for next scene , including the main method, speech synthesis, imports, and constant definitions.Please ensure that the generated code for the next scene is synchronized with the previous scene's code in terms of interface style, coordinate system, and elements.If there is no next scene, output done.
```
...
class CombinedScene(Scene):
    ...
    def play_section_02(self):
        ...
        gravity_arrow_feather = Arrow(
            start=feather.get_center() + UP * 0.2, # Start slightly above center
            end=feather.get_center() + DOWN * 1.5, # Point downwards
        ...
        # Gravity Label
        gravity_label = Text("重力 (Gravity)", font_size=24, color=MY_GRAVITY_RED)
        gravity_label.next_to(gravity_arrow_stone, RIGHT, buff=SMALL_BUFF)

        gravity_elements = VGroup(gravity_arrow_feather, gravity_arrow_stone, gravity_label)
...
MY_AIR_PARTICLE_COLOR = "#B0BEC5" # Subtle color for air particles
...
class CombinedScene(Scene):
    ...
    def play_section_03(self):
        ...
        air_res_expl = Text("物体在空气中下落时，会受到空气向上的阻力。", font_size=32, color=MY_DARK_TEXT, width=config.frame_width - 4) # Wrap text
        ...
        gravity_arrow_feather = Arrow(
            ...
            end=feather.get_center() + DOWN * 1.0, # Shorter gravity arrow
            color=MY_GRAVITY_RED,
            buff=0.1, stroke_width=5, max_tip_length_to_length_ratio=0.2
        ).set_opacity(0.7)
        gravity_arrow_stone = Arrow(
            start=stone.get_center() + UP * 0.1,
            end=stone.get_center() + DOWN * 1.0, # Shorter gravity arrow
            color=MY_GRAVITY_RED,
            buff=0.1, stroke_width=5, max_tip_length_to_length_ratio=0.2
        ).set_opacity(0.7)
        ...
        # Feather: Large surface area, light -> significant air resistance
        air_res_arrow_feather = Arrow(
            start=feather.get_center() + DOWN * 0.8, # Start below center
            end=feather.get_center() + UP * 1.2,    # Point upwards, longer than gravity
            color=MY_AIR_RESISTANCE_BLUE,
            buff=0.1,
            stroke_width=6,
            max_tip_length_to_length_ratio=0.2
        )
        # Stone: Smaller surface area relative to weight -> less significant air resistance
        air_res_arrow_stone = Arrow(
            start=stone.get_center() + DOWN * 0.8,
            end=stone.get_center() + UP * 0.4,     # Point upwards, much shorter
            color=MY_AIR_RESISTANCE_BLUE,
            buff=0.1,
            stroke_width=6,
            max_tip_length_to_length_ratio=0.2
        ...
        air_particles = VGroup(*[
            Dot(point=[np.random.uniform(-config.frame_width/2+1, config.frame_width/2-1),
                       ...
                       0],
                radius=0.02, color=MY_AIR_PARTICLE_COLOR, fill_opacity=0.5)
            for _ in range(50)
        ]).set_z_index(-1) # Behind other elements
        ...
        with custom_voiceover_tts(voice_text_03) as tracker:
            ...
            # Animation Sequence
            self.play(FadeIn(text_group), FadeIn(subtitle_voice), run_time=1.5)
            # Fade in objects and gravity (assuming they might not be present from prev scene in combined video)
            self.play(FadeIn(feather), FadeIn(stone), FadeIn(gravity_elements), FadeIn(air_particles), run_time=1.0)
            # Create Air Resistance Arrows
            self.play(
...
MY_GRAVITY_RED = "#E53935" # Color for gravity arrows
MY_AIR_RESISTANCE_BLUE = "#03A9F4" # Light blue for air resistance arrows
MY_NET_FORCE_GREEN = "#4CAF50" # Green for net force arrows
MY_GROUND_BROWN = "#A1887F" # Color for the ground
...
class CombinedScene(Scene):
    """
    Explains why feathers fall slower than stones.
    Scene 4: Effect of Forces and the Fall.
    Uses Chinese narration and subtitles.
    """
    def setup(self):
        Scene.setup(self)
    ...
    # --- Section 4: Effect of Forces and the Fall ---
    def play_section_04(self):
        """Section 4: Show the effect of forces and the actual fall."""
        ...
        # --- Forces Visualization (Static representation for clarity) ---
        # Gravity Arrows
        gravity_feather = Arrow(feather.get_center(), feather.get_center() + DOWN * 1.0, color=MY_GRAVITY_RED, buff=0.1, stroke_width=5)
        gravity_stone = Arrow(stone.get_center(), stone.get_center() + DOWN * 1.5, color=MY_GRAVITY_RED, buff=0.1, stroke_width=5) # Stone has more weight/gravity force

        # Air Resistance Arrows
        air_res_feather = Arrow(feather.get_center(), feather.get_center() + UP * 0.9, color=MY_AIR_RESISTANCE_BLUE, buff=0.1, stroke_width=5) # Almost balances gravity
        air_res_stone = Arrow(stone.get_center(), stone.get_center() + UP * 0.3, color=MY_AIR_RESISTANCE_BLUE, buff=0.1, stroke_width=5) # Small air resistance

        # Net Force Arrows (Gravity - Air Resistance)
        net_force_feather = Arrow(feather.get_center()+DOWN*0.1, feather.get_center() + DOWN * 0.2, color=MY_NET_FORCE_GREEN, buff=0.1, stroke_width=6) # Very small net force
        net_force_stone = Arrow(stone.get_center()+DOWN*0.1, stone.get_center() + DOWN * 1.3, color=MY_NET_FORCE_GREEN, buff=0.1, stroke_width=6) # Large net force
        ...
        with custom_voiceover_tts(voice_text_04) as tracker:
            ...
            # Animation Sequence
            self.play(FadeIn(title), FadeIn(ground), FadeIn(subtitle_voice), run_time=1.0)
            self.play(FadeIn(feather), FadeIn(stone), run_time=0.5)
            # Show forces and explanation
            self.play(FadeIn(explanation), Create(forces_group), run_time=3.0)
            self.wait(6.0) # Wait for explanation of forces

            # Animate the fall
            self.play(FadeOut(forces_group), FadeOut(explanation), run_time=0.5) # Clear forces before fall
            ...
            # Feather falls slow and drifts slightly
            feather_target_pos = feather_start_pos + DOWN * (start_y - ground.get_y() - feather.height/2) * 0.6 + RIGHT * 0.5 # Doesn't reach ground, drifts right
            ...
            # Synchronization
            anim_duration = 1.0 + 0.5 + 3.0 + 6.0 + 0.5 + 4.0 # Includes waits and fall time
            remaining_wait_time = max(0, tracker.duration - anim_duration - 0.5) if tracker.duration > 0 else 1.0
...
MY_NET_FORCE_GREEN = "#4CAF50" # Green for net force arrows
MY_GROUND_BROWN = "#A1887F" # Color for the ground
MY_VACUUM_BG = "#37474F" # Darker background for vacuum
MY_VACUUM_BORDER = "#B0BEC5" # Border for vacuum chamber
...
class CombinedScene(Scene):
    """
    Explains why feathers fall slower than stones.
    Scene 5: What happens in a vacuum.
    Uses Chinese narration and subtitles.
    """
    def setup(self):
        Scene.setup(self)
        ...
        self.section_elements = VGroup()
        # Set background color for the entire scene
        # Keep sky blue for consistency, or change to vacuum color? Let's change.
        self.camera.background_color = MY_VACUUM_BG

    def update_scene_number(self, number_str, color=MY_WHITE): # Default to white text on dark BG
        """Fades out the old scene number and fades in the new one."""
        new_scene_num = Text(number_str, font_size=24, color=color).to_corner(UR, buff=MED_LARGE_BUFF).set_z_index(10)
        animations = [FadeIn(new_scene_num, run_time=0.5)]
        if self.current_scene_num_mob:
    ...
    def play_section_05(self):
        ...
        # Title
        title = Text("在真空中会怎样？ (In a Vacuum?)", font_size=40, color=MY_WHITE) # White text
        title.to_edge(UP, buff=MED_LARGE_BUFF)
        self.section_elements.add(title)
        ...
        chamber_label = Text("真空环境 (Vacuum)", font_size=24, color=MY_WHITE).next_to(vacuum_chamber, DOWN) # White text
        chamber_group = VGroup(vacuum_chamber, chamber_label)
        self.section_elements.add(chamber_group)

        # Starting Positions (Inside the chamber, near the top)
        ...
        feather_shape = Polygon(
            ...
            [0.2, 1, 0], [0.4, 0, 0], [0.5, -1.5, 0],
            color=MY_FEATHER_WHITE, fill_opacity=1.0, stroke_width=1, stroke_color=MY_DARK_TEXT
        ).scale(0.6).rotate(-PI / 12) # Slightly smaller for chamber
        feather = VGroup(feather_shape).move_to(feather_start_pos)
        self.section_elements.add(feather)
        ...
        stone_shape = Polygon(
            ...
            [0.1, 0.9, 0], [-0.7, 0.5, 0],
            color=MY_STONE_GRAY, fill_opacity=1.0, stroke_width=0
        ).scale(0.9) # Slightly smaller for chamber
        stone = VGroup(stone_shape).move_to(stone_start_pos)
        self.section_elements.add(stone)

        # Gravity Arrows Only
        gravity_feather = Arrow(feather.get_center(), feather.get_center() + DOWN * 1.0, color=MY_GRAVITY_RED, buff=0.1, stroke_width=5)
        gravity_stone = Arrow(stone.get_center(), stone.get_center() + DOWN * 1.0, color=MY_GRAVITY_RED, buff=0.1, stroke_width=5) # Same length arrow visually represents same acceleration
        ...
        # Add forces group later during animation

        # Narration 5 (Chinese)
        voice_text_05 = "那么，如果在一个没有空气，也就是真空的环境里，情况会怎样呢？在真空中，没有空气阻力。唯一作用在物体上的力就是重力。根据物理学原理，在只受重力作用的情况下，所有物体，无论轻重、形状如何，都会以相同的加速度下落。"
        with custom_voiceover_tts(voice_text_05) as tracker:
            ...
            subtitle_voice = Text(
                voice_text_05,
                font_size=32,
                color=MY_WHITE, # White text on dark background
                width=config.frame_width - 2, # Wrap text
                should_center=True
            ).to_edge(DOWN, buff=MED_SMALL_BUFF)
            ...
            self.wait(5.0) # Wait for explanation

            # Animate the fall in vacuum
            self.play(FadeOut(gravity_feather), FadeOut(gravity_stone), FadeOut(gravity_label), run_time=0.5) # Remove forces before fall

            target_y = vacuum_chamber.get_bottom()[1] + 0.5 # Target y slightly above bottom
            feather_target_pos = feather.get_center() * [1,0,0] + target_y * UP
            stone_target_pos = stone.get_center() * [1,0,0] + target_y * UP

            # They fall together - same duration, same easing
            fall_duration = 2.5
...
class CombinedScene(Scene):
    ...
    def setup(self):
        ...
        # Store elements needed across sections
        self.section_elements = VGroup()
        # Set background color for the entire scene - back to sky blue for summary
        self.camera.background_color = MY_SKY_BLUE

    def update_scene_number(self, number_str, color=MY_DARK_TEXT): # Default dark text on light BG
        """Fades out the old scene number and fades in the new one."""
        new_scene_num = Text(number_str, font_size=24, color=color).to_corner(UR, buff=MED_LARGE_BUFF).set_z_index(10)
        animations = [FadeIn(new_scene_num, run_time=0.5)]
        if self.current_scene_num_mob:
    ...
    def play_section_06(self):
        ...
        self.update_scene_number("06") # Update scene number
        ...
        # Summary Points
        point_size = 32
        line_width = config.frame_width - 4 # Allow text wrapping
        ...
        # Align the whole group to the left edge for better readability
        ...
        with custom_voiceover_tts(voice_text_06) as tracker:
            ...
            # Animate summary points appearing one by one
