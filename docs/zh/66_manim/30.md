...
### The title must be placed in the title area.

The title must be placed in the title area.
```python
title = Title("Trigonometric Identity")
layout["title"].place(title)
```
### Text
All text should be placed in the left-side text area.
```python
text = Text("...")
layout["title"].place(text)
```
...
## Common Manim Errors
* Error: The regular `Scene`’s `Camera` has no `frame` attribute; to move/zoom/rotate the camera, inherit from `MovingCameraScene`.
* Error: `plot()`’s `x_range` can only be `[min, max]` or `[min, max, step]`; you can’t use four values to skip a discontinuity. For discontinuous functions, call `axes.plot(x_range=…)` for each continuous segment and combine them with a `VGroup`.
* Error: Python string literals must use straight quotes (`'` or `"`), not curly/typographic quotes. When copying code, replace all “”“‘’” with straight quotes, and escape with `\"` or `\'` if needed.
* Error: `interpolate_color()` only accepts Manim color objects or RGB values, not hex-string literals (e.g. `"#800020"`). Use built-in color constants or generate colors via `rgb_to_color()`/`color_gradient()`.
* Error: Loading a nonexistent file with `ImageMobject("…")` raises an `OSError`; before use, check the path or wrap in `try/except`, and prefer programmatic shapes (e.g. `VMobject`/`Polygon`) instead of external images.
* Error: You cannot directly compare NumPy arrays (like `RIGHT`) with `==` in a conditional. Use string identifiers (e.g. `"right"`) for comparisons, or `np.array_equal()` to test array equality.
* Error: `Sector` objects have no `point_at_angle()` method. To get points on a sector’s perimeter, use an `Arc`’s `point_from_proportion()` or compute coordinates manually with trigonometry.
* Error: The `Sector` constructor does not accept an `outer_radius` parameter; use `radius` to set the sector’s size and always verify parameter names against your Manim version’s API.
* Error: `Arc` does not support a `dash_length` parameter. To draw a dashed arc, first create a regular `Arc`, then wrap it in `DashedVMobject(arc, num_dashes=…, dash_spacing=…)`.
* Error: Do not use undefined constants like `CENTER` or `MIDPOINT` in ManimCE v0.19.0. For centering, use `ORIGIN` (or `[0,0,0]`); for directions, use the built-in constants `UP`, `DOWN`, `LEFT`, `RIGHT`, `UL`, `UR`, `DL`, `DR`.
* Error: Manim has no `Check` or `Cross` classes. To display checkmarks or crosses, use `Text("√")` and `Text("×")` instead.
