...
2. **上传 MP4 场景文件（上传接口 /hls/upload）**  
   客户端依次将由 Manim 生成的每个场景的 MP4 文件上传至服务器，上传时需附带唯一会话编号（sessionId）和（可选）场景序号。
...
- **接口目的**
  ...
  - 会话初始化：客户端在上传第一个场景前调用该接口，提交唯一的 sessionId,雪花格式，服务器据此创建新的播放会话，包括初始化一个空的播放列表文件。
...
- **请求参数**
  ...
  - `sessionId`（必填）：字符串，客户端生成的唯一标识。
  - `startTime`（可选）：ISO 8601 格式的时间字符串（用于日志记录和统计）。

  请求示例（JSON 格式）：

  ```json
  {
    "sessionId": "abc123-xxxx",
...
- **返回值**
  ...
  ```json
  {
    ...
    "streamUrl": "http://your-server/hls/{sessionId}/playlist.m3u8"
  }
  ```

- **服务端内部逻辑**
  - 根据 `sessionId` 在预定义目录下创建会话文件夹（例如 `/data/hls/{sessionId}/`）。
...
- **请求参数**
  ...
  - `sessionId`（必填）：客户端生成的唯一编号，用于标识会话/流。
  - `sceneIndex`（可选）：当前场景的序号，便于服务器按顺序处理（若不传，服务器可根据上传时间排序）。
  - 文件数据：Content-Type 为 multipart/form-data，包含 MP4 文件。

  请求示例：

  ```
  POST /upload?sessionId={uniqueId}
...
- **请求参数**
  ...
  - `sessionId`（必填）：对应的会话编号。

- **返回值**
  ```json
  {
    "status": "success",
    "streamUrl": "http://your-server/hls/abc123/playlist.m3u8"
  }
  ```
...
- **返回值**
  ```json
  {
    ...
    "sessionId": "abc123-xxxx",
    "processedScenes": 3,
    "currentMediaSequence": 18,
    "lastUpdateTime": "2025-03-30T11:20:00Z"
  }
  ```
...
- **请求参数**
  ...
  - `sessionId`（必填）：当前播放流的唯一标识，与上传接口中一致。
  - `finishTime`（可选）：ISO 8601 格式时间字符串，用于记录日志和统计。
  ...
  {
    "sessionId": "abc123-xxxx",
    "finishTime": "2025-03-30T12:00:00Z"
  }
  ```
...
- **返回值**
  ...
  {
    "status": "success",
    "message": "Playback stream marked as finished."
  }
  ```

- **服务端内部逻辑**
  1. 接收结束通知，根据 `sessionId` 定位到对应会话的播放列表管理模块。
...
- **唯一标识管理**  
  根据上传请求中的 `sessionId`，将同一会话的 MP4 文件存放到特定目录或队列中，例如：  
  `/data/uploads/{sessionId}/scene_{sceneIndex}.mp4`
...
- **列表文件更新**  
  定时或实时将内存中的播放列表写入磁盘（例如 `/data/hls/{sessionId}/playlist.m3u8`），供外部 HTTP 服务器访问，或通过 API 将最新列表返回给客户端。
