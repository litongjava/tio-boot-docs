...
## diagram
* If you choose a left-right layout and assign the right side as the diagram region, it’s recommended to display only a single main diagram; too many elements will look crowded and confusing.
* All label fonts within the diagram region should be black to ensure they’re clearly visible on screen.
* If you need to highlight important information below the diagram, use bold text with a border—there’s no need to add a background color to the text.
* When illustrating proof diagrams, all auxiliary lines should be rendered as dashed lines.
* When referring to a specific angle such as ∠ABC, that angle must be clearly marked in the diagram.
...
### How to Correctly Use `place` to Align the Entire Diagram and Subsequent Elements

You must follow these guidelines to ensure the main diagram **and** any later-added elements (axes, curves, labels, legend, angle arcs, highlight circles, etc.) are all scaled and translated together into the target layout region.
...
1. **Pack Everything into a Single Group**
   ...
   * **Shaded regions**: `upper_trap`, `lower_trap` – filled polygons showing areas of interest
   ...
   ```
Before the call to `layout.place`, add all elements—including the diagram, parallel indicators, filled trapezoids, and any later-appearing highlight/illustration objects drawn directly—to a single `VGroup`, then call only once:

When constructing `diagram_group`, be sure to include **all** elements you will display later (e.g., `midline` and `midline_label`) in the **same** `VGroup`, and complete that grouping **before** the **first** call to 

You must ensure that in the code executed after `custom_voiceover_tts`, every Mobject played via `self.play` is already included in the same `diagram_group`. Any objects that will be used dynamically later in the scene must be added to `diagram_group` before invoking `custom_voiceover_tts`, so that a single call to `layout.place` can correctly scale and align all elements.


  ```python
  diagram_group = VGroup(...all diagram elements...)
  layout["body"]["diagram"].place(diagram_group, aligned_edge=…)
  ```
If you absolutely need to create an Mobject after `custom_voiceover_tts` and it must still appear on screen, you must use `diagram.get_center()` to obtain the center coordinates as a reference for recalculating the new Mobject’s position.
...
### Include Every Related Element in the Diagram Group Before Placing

When building your `diagram_group`, make sure to add **all** elements that belong to the diagram—including even the auxiliary lines (`line_AB`, `line_AD`, `line_BC`, `line_CD`, etc.)—into the same group. Otherwise, after you call

```python
layout["body"]["diagram"].place(diagram_group)
```

to scale and position the group, any lines left out of `diagram_group` will remain at their original coordinates, causing the layout to break.
...
class CombinedScene(Scene):
    ...
    def play_scene_01(self):
        ...
        layout = Layout(LayoutDirection.VERTICAL, {
            ...
            "content_area": (8.0, Layout(LayoutDirection.HORIZONTAL, {
                ...
                "right_diagram_area": (5.5, LayoutAtom())
            }))
        }).resolve(self)
        ...
        # Create the base diagram elements - these use original coordinates because they're part of the diagram
        ...
        diagram = VGroup(poly_ABCD, line_BE, line_DF, label_A, label_B, label_C, label_D, label_E, label_F)
        layout["content_area"]["right_diagram_area"].place(diagram)

        # Store the coordinates after diagram placement
        ...
        diagram_center = diagram.get_center()
        A_actual = A_pt + diagram_center
        B_actual = B_pt + diagram_center
        C_actual = C_pt + diagram_center
        D_actual = D_pt + diagram_center
        E_actual = E_pt + diagram_center
        F_actual = F_pt + diagram_center
        # Save actual points
        ...
        with custom_voiceover_tts(voice_text_problem) as tracker:
            ...
            self.play(FadeIn(problem_statement_group, shift=RIGHT), Create(diagram), run_time=2.0)
    ...
    def play_scene_02(self):
        ...
        # This ensures highlights appear exactly on top of the diagram elements
        ...
        with custom_voiceover_tts(voice_step3) as tracker:
            ...
            # Always use A_actual, B_actual etc. for any objects that need to align with the displayed diagram
            # This ensures the triangles appear exactly on top of the intended diagram vertices
            ...
            # causing them to appear offset from the diagram
