...
要使用 JAVE 提取视频中的音频，可以按照以下步骤操作：

#### a. 添加 Maven 依赖

首先，在的 `pom.xml` 文件中添加 JAVE 的依赖。`jave-all-deps` 是一个依赖项较多的库，请根据实际需求谨慎使用。
...
```java
package nexus.io.jave.server.controller;
...
import nexus.io.jave.server.service.MediaService;
import nexus.io.jave.server.service.VideoService;
...
```java
package nexus.io.jave.server.service;
