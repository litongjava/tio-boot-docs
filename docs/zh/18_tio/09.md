...
- **设置属性**：使用 `ChannelContext.set(String key, Object value)` 方法可以将业务数据存储在 `ChannelContext` 中。这个方法允许在 `ChannelContext` 对象中保存任意键值对。
...
`Tio.bindUser(ChannelContext channelContext, String userid)` 允许将 `userid` 与 `ChannelContext` 对象绑定起来。一旦绑定后，可以通过 `userid` 来执行各种操作，例如：
...
可以使用 `Tio.bindBsId(ChannelContext channelContext, String bsId)` 方法将一个业务 ID 与 `ChannelContext` 绑定。这在需要根据业务 ID 区分不同连接的场景中非常有用。
...
`Tio.bindGroup(ChannelContext channelContext, String group)` 方法允许将 `ChannelContext` 绑定到一个群组。绑定后，可以通过群组 ID 向群组内的所有连接发送广播消息。
...
希望这段文档能帮助更好地理解 `ChannelContext` 对象的使用。
