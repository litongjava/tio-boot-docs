...
- 如果使用 `tio` 作为 TCP 客户端，则需要创建 `ClientTioConfig` 对象。
- 如果使用 `tio` 作为 TCP 服务器，则需要创建 `ServerTioConfig` 对象。
...
如果已经获取到 `ChannelContext` 对象，可以直接通过以下代码获取 `TioConfig`：
...
也可以通过 `TioBootServer` 对象获取 `ServerTioConfig`，方法如下：
