...
`Packet` 是用于表示业务数据结构的类。在使用 `t-io` 时，通常会通过继承 `Packet` 类来实现自己的业务数据结构。可以把 `Packet` 看作一个普通的 VO（Value Object）对象，用于传递数据。
...
`AioHandler` 是处理消息的核心接口。当使用 `t-io` 作为 TCP 客户端时，需要实现 `ClientAioHandler`；当使用 `t-io` 作为 TCP 服务器时，需要实现 `ServerAioHandler`。`AioHandler` 接口定义了三个重要的方法：
...
`AioListener` 是监听消息处理的核心接口。当使用 `t-io` 作为 TCP 客户端时，需要实现 `ClientAioListener`；当使用 `t-io` 作为 TCP 服务器时，需要实现 `ServerAioListener`。它定义了处理连接和消息生命周期的各个回调方法，如连接建立、消息解码、消息发送、消息处理等。
