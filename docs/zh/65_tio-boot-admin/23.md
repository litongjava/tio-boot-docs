...
一篇文章可以有多张图片；删除文章中的图片关联，不等于删除 OSS 对象。文件表设计和扩容说明见[文件信息表](../64_文件存储/01.md)，统一上传服务见[文件上传](06.md)。
...
一个用户只需要一个当前头像时，可以在用户表增加 `avatar_file_id BIGINT`，逻辑关联统一上传表的 `id`，无需为当前头像单独建立多图片关联表。需要头像历史、审核或多个版本时，再引入关联表。

前端先选图并上传，获得文件记录 ID 和预览地址；保存个人资料时只提交 `avatar_file_id`。后端根据当前登录用户校验文件归属、未删除状态和已验证的图片类型，在事务中保存资料与头像关联。校验失败时不能部分更新昵称等其他字段。

建议明确更新语义：不传该字段表示保留，传 `null` 表示清空，传有效 ID 表示替换。不要接受客户端提供的任意头像 URL。历史地址可作为兼容字段，有文件关联时优先根据关联生成展示 URL；替换头像时清空历史地址，避免清空关联后意外恢复旧头像。

读取个人资料、公开主页、文章作者和会话列表时，通过 `UniStorageService.getPresignedDownloadUrl(...)` 获取展示地址。统一处理这些入口，避免个人资料更新后其他页面仍显示旧头像。前端上传期间禁用保存，选图返回页面时避免重新加载资料覆盖尚未保存的预览。

替换或清空头像仅修改关联，不立即删除对象。清理上传文件前应同时检查文章图片、用户头像等所有有效引用。验证应覆盖正常替换、跨用户引用拒绝、无效文件拒绝、失败回滚、保留和清空语义，以及保存后重新读取头像。
