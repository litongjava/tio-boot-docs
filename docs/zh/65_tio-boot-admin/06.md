...
手动创建 `tio_boot_admin_system_upload_file`，参见[文件信息表](../64_文件存储/01.md)。已建表的应用无需重新建表。
...
| 配置值 | 平台配置及上传说明 |
| --- | --- |
| `aliyun_oss` | [阿里云 OSS](../64_文件存储/06.md) |
| `aws_s3` | [亚马逊 S3](../64_文件存储/03.md) |
| `cloudflare_r2` | [Cloudflare R2](../64_文件存储/04.md) |
| `tencent_cos` | [腾讯 COS](../64_文件存储/05.md) |
...
原名上限 255，分类上限 128；后缀不再单独限制为 10 位，而是受完整文件名上限约束。禁止文件名携带目录分隔符、控制字符，分类只接受字母、数字、下划线和短横线。表的 `target_name` 扩容为 1024，为生成的对象路径留出空间。已有表先执行[文件表扩容 SQL](../64_文件存储/01.md)，再启用新长度；HTTP 请求体大小限制仍由框架配置控制。
