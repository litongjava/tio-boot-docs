...
`findFirstMap(sql, paras)` 返回 `Kv`，没有 JSON 列需要解析时直接传 SQL 参数；需要解析 JSON 列时使用 `findFirstMap(sql, jsonFields, parameters)`。查不到记录返回 `null`，调用 getter 前应先判空。可使用 `result.getStr("database_name")`、`result.getInt("value")` 读取类型明确的值。SQL 参数使用 `?` 绑定，不拼接用户输入。
