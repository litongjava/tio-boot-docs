...
public class MyStaticResourceHandler implements StaticResourceHandler {
  ...
  public HttpResponse handle(String path, HttpRequest request, HttpConfig httpConfig, AbsCache staticResCache) {
    try {
      ...
      HttpResponse response = buildFileResponse(httpResource, request);
  ...
  private HttpResponse buildFileResponse(HttpResource httpResource, HttpRequest request) throws IOException {
    byte[] fileContent = renderHttpResources(request.getDomain(), httpResource);
    File file = httpResource.getFile();
    String extension = getFileExtension(file.getName());

    HttpResponse response = Resps.bytes(request, fileContent, extension);
    response.setStaticRes(true);
    response.setLastModified(HeaderValue.from(String.valueOf(file.lastModified())));
    return response;
  }

  private byte[] renderHttpResources(String domain, HttpResource httpResource) throws IOException {
    File file = httpResource.getFile();
