...
import nexus.io.tio.web.hello.handler.MyDecodeExceptionHandler;
...
- **导入**：引入必要的类和注解。
- **注解**：
  - `@AopBeforeStartConfiguration`：标识此配置应在服务器启动前应用。
...
TioDecodeException: nexus.io.tio.core.exception.TioDecodeException: 1==1 contain multi=
```
...
import nexus.io.constatns.ServerConfigKeys;
...
- **注解**：
  - `@AopBeforeStartConfiguration`：确保配置在服务器初始化前应用。
