...
5. **decodeErrorCount (AtomicInteger decodeErrorCount)**  
   解码错误的次数。如果某个 IP 在解码请求数据时频繁出错，可能表明该 IP 存在潜在的攻击行为或其他异常。
...
15. **getBytesPerTcpReceive()**  
    计算每个 TCP 连接接收到的平均字节数。可以用于检测慢速攻击（Slow Attack），如果每个 TCP 包接收到的字节数特别少，可能表明潜在的慢速攻击。

16. **getPacketsPerTcpReceive()**  
    计算每个 TCP 连接接收到的业务包的平均数量。与慢速攻击检测相关，如果该值较低，可能表明攻击行为。
...
```log
2024-09-08 11:59:08.444 [tio-group-2] INFO  c.l.t.w.h.s.MyIpStatListener.onAfterConnected:30 - onAfterConnected
{"duration":"-9","bytesPerTcpReceive":0.0,"packetsPerTcpReceive":0.0,"handledPackets":0,"receivedPackets":0,"handledPacketCosts":0,"handledCostsPerPacket":0.0,"ip":"0:0:0:0:0:0:0:1","handledBytes":0,"receivedBytes":0,"sentPackets":0,"sentBytes":0,"receivedTcps":0,"requestCount":1,"start":"2024-09-08 11:59:08","decodeErrorCount":0,"formatedDuration":"0毫秒","durationType":"300"}
2024-09-08 11:59:08.449 [tio-group-3] INFO  c.l.t.w.h.s.MyIpStatListener.onAfterReceivedBytes:58 - onAfterReceivedBytes
{"duration":"6","bytesPerTcpReceive":1495.0,"packetsPerTcpReceive":0.0,"handledPackets":0,"receivedPackets":0,"handledPacketCosts":0,"handledCostsPerPacket":0.0,"ip":"0:0:0:0:0:0:0:1","handledBytes":0,"receivedBytes":1495,"sentPackets":0,"sentBytes":0,"receivedTcps":1,"requestCount":1,"start":"2024-09-08 11:59:08","decodeErrorCount":0,"formatedDuration":"6毫秒","durationType":"300"}
2024-09-08 11:59:08.459 [tio-group-3] INFO  c.l.t.w.h.s.MyIpStatListener.onAfterDecoded:51 - onAfterDecoded
request ID_1

{"duration":"6","bytesPerTcpReceive":1495.0,"packetsPerTcpReceive":1.0,"handledPackets":0,"receivedPackets":1,"handledPacketCosts":0,"handledCostsPerPacket":0.0,"ip":"0:0:0:0:0:0:0:1","handledBytes":0,"receivedBytes":1495,"sentPackets":0,"sentBytes":0,"receivedTcps":1,"requestCount":1,"start":"2024-09-08 11:59:08","decodeErrorCount":0,"formatedDuration":"6毫秒","durationType":"300"}
2024-09-08 11:59:08.644 [tio-group-3] INFO  c.l.t.b.h.h.DefaultHttpRequestHandler.handler:317 -
-----------action report---------------------
request:GET / HTTP/1.1
method:public com.litongjava.tio.utils.resp.RespVo com.litongjava.tio.web.hello.controller.IndexController.index()

2024-09-08 11:59:08.660 [tio-group-3] INFO  c.l.t.w.h.s.MyIpStatListener.onAfterHandled:65 - onAfterHandled
request ID_1

{"duration":"209","bytesPerTcpReceive":1495.0,"packetsPerTcpReceive":1.0,"handledPackets":1,"receivedPackets":1,"handledPacketCosts":203,"handledCostsPerPacket":203.0,"ip":"0:0:0:0:0:0:0:1","handledBytes":1495,"receivedBytes":1495,"sentPackets":0,"sentBytes":0,"receivedTcps":1,"requestCount":1,"start":"2024-09-08 11:59:08","decodeErrorCount":0,"formatedDuration":"209毫秒","durationType":"300"}
2024-09-08 11:59:08.664 [tio-group-4] INFO  c.l.t.w.h.s.MyIpStatListener.onAfterSent:44 - onAfterSent
reponse: requestID_1  /
...
4. **业务逻辑处理**
   ...
   - **触发时机**：服务器处理解码后的业务请求，此阶段可能涉及控制器处理请求。
...
{
  "duration":"-9",
  "bytesPerTcpReceive":0.0,
  "packetsPerTcpReceive":0.0,
  "handledPackets":0,
  "receivedPackets":0,
  "handledPacketCosts":0,
  "handledCostsPerPacket":0.0,
  "ip":"0:0:0:0:0:0:0:1",
  "handledBytes":0,
  "receivedBytes":0,
  "sentPackets":0,
  ...
  "receivedTcps":0,
  "requestCount":1,
  "start":"2024-09-08 11:59:08",
  "decodeErrorCount":0,
  "formatedDuration":"0毫秒",
  "durationType":"300"
}
```

- **时间戳**：`2024-09-08 11:59:08.444`
- **线程**：`[tio-group-2]`
- **日志级别**：`INFO`
- **类与方法**：`c.l.t.w.h.s.MyIpStatListener.onAfterConnected:30`
- **日志信息**：`onAfterConnected`
- **数据内容**：
  - `ip`: 客户端 IP 地址，`0:0:0:0:0:0:0:1` 表示本地回环地址（IPv6）。
  - `requestCount`: 连接请求次数，值为 `1`。
  - `start`: 统计开始时间，`2024-09-08 11:59:08`。
  - 其他字段（如 `bytesPerTcpReceive`, `packetsPerTcpReceive` 等）初始值为 `0`，因为此时尚未有数据传输。

**含义**：客户端成功与服务器建立连接，初始化了 IP 统计数据。
...
2024-09-08 11:59:08.449 [tio-group-3] INFO  c.l.t.w.h.s.MyIpStatListener.onAfterReceivedBytes:58 - onAfterReceivedBytes
{
  ...
  "start":"2024-09-08 11:59:08",
  "decodeErrorCount":0,
  "formatedDuration":"6毫秒",
  "durationType":"300"
}
```

- **时间戳**：`2024-09-08 11:59:08.449`
- **线程**：`[tio-group-3]`
- **日志级别**：`INFO`
- **类与方法**：`c.l.t.w.h.s.MyIpStatListener.onAfterReceivedBytes:58`
- **日志信息**：`onAfterReceivedBytes`
- **数据内容**：
  - `duration`: 统计持续时间，`6` 毫秒。
  - `bytesPerTcpReceive`: 每个 TCP 接收的平均字节数，`1495.0`。
  - `receivedBytes`: 接收到的总字节数，`1495`。
  - `receivedTcps`: 接收到的 TCP 数据包数，`1`。
  - 其他字段仍为初始或未更新状态。

**含义**：服务器接收到来自客户端的 `1495` 字节数据，来自 `1` 个 TCP 数据包，统计持续时间为 `6` 毫秒。
...
- **时间戳**：`2024-09-08 11:59:08.459`
- **线程**：`[tio-group-3]`
- **日志级别**：`INFO`
- **类与方法**：`c.l.t.w.h.s.MyIpStatListener.onAfterDecoded:51`
- **日志信息**：
  - `onAfterDecoded`
  - **附加信息**：`request ID_1`
- **数据内容**：
  - `packetsPerTcpReceive`: 每个 TCP 接收的业务包数量，`1.0`。
  - `receivedPackets`: 接收到的业务包数量，`1`。
  - 其他字段保持不变。

**含义**：服务器成功解码了来自客户端的 `1` 个业务数据包，统计持续时间仍为 `6` 毫秒。
...
- **时间戳**：`2024-09-08 11:59:08.644`
- **线程**：`[tio-group-3]`
- **日志级别**：`INFO`
- **类与方法**：`c.l.t.b.h.h.DefaultHttpRequestHandler.handler:317`
- **日志信息**：
  - **Action Report**：
    - `request`: HTTP 请求信息，`GET / HTTP/1.1`。
    - `method`: 处理该请求的控制器方法，`IndexController.index()`。

**含义**：服务器接收到一个 `GET /` 请求，并通过 `IndexController.index()` 方法进行处理。
...

{
  ...
  "handledPackets":1,
  "receivedPackets":1,
  "handledPacketCosts":203,
  "handledCostsPerPacket":203.0,
  "ip":"0:0:0:0:0:0:0:1",
  "handledBytes":1495,
  "receivedBytes":1495,
  "sentPackets":0,
  ...
  "start":"2024-09-08 11:59:08",
  "decodeErrorCount":0,
  "formatedDuration":"209毫秒",
  "durationType":"300"
}
```

- **时间戳**：`2024-09-08 11:59:08.660`
- **线程**：`[tio-group-3]`
- **日志级别**：`INFO`
- **类与方法**：`c.l.t.w.h.s.MyIpStatListener.onAfterHandled:65`
- **日志信息**：
  - `onAfterHandled`
  - **附加信息**：`request ID_1`
- **数据内容**：
  - `duration`: 统计持续时间，`209` 毫秒。
  - `handledPackets`: 处理的消息包数量，`1`。
  - `handledPacketCosts`: 处理消息包的总耗时，`203` 毫秒。
  - `handledCostsPerPacket`: 每个消息包的平均处理耗时，`203.0` 毫秒。
  - `handledBytes`: 处理的字节数，`1495`。
  - `formatedDuration`: 格式化的统计持续时间，`209毫秒`。
  - 其他字段保持不变。

**含义**：服务器完成了对 `request ID_1` 的业务逻辑处理，处理耗时 `203` 毫秒，总统计持续时间为 `209` 毫秒。
...
- **时间戳**：`2024-09-08 11:59:08.664`
- **线程**：`[tio-group-4]`
- **日志级别**：`INFO`
- **类与方法**：`c.l.t.w.h.s.MyIpStatListener.onAfterSent:44`
- **日志信息**：
  - `onAfterSent`
  - **附加信息**：`response: requestID_1  /`
- **数据内容**：日志中未包含详细的统计数据，仅记录了响应信息。

**含义**：服务器完成了对 `requestID_1` 的响应发送，响应的目标路径为 `/`。
