...
[[toc]]
TQL（Table SQL）是一种以 http 请求参数 为载体的结构化查询语言，用于操作数据表。
...
查询操作符用于对数据字段进行操作，执行的查询指令如下。具体实现可参考源码 `nexus.io.data.services.OperatorService`。
...
### 类型指令
类型指令用于解析数据字段的值。例如，如果数据库的主键类型Long,通常使用雪花 ID（Snowflake ID）.后端会Long转为String发送到前端,前端通常会以字符串（string）类型传递该值，后端在获取值后将其转换为长整型（long）类型。类型指令通过指定字段类型，确保前端、后端和数据库的数据字段类型一致。

为了减少前端的记忆负担,新版本后端会进行类型转换,前端无需手动指定字段的类型
#### 指令说明
