...
```java
package nexus.io.log.server.handler;

import java.nio.ByteBuffer;

import nexus.io.aio.Packet;
import nexus.io.log.server.packet.DemoPacket;
...
```java
package nexus.io.log.server.listener;
...
```java
package nexus.io.log.server.config;

import nexus.io.annotation.BeforeStartConfiguration;
import nexus.io.annotation.Initialization;
import nexus.io.log.server.handler.DemoHandler;
import nexus.io.log.server.listener.DemoListener;
...
2025-01-23 20:37:04.433 [Thread-15] INFO   c.l.l.s.l.DemoListener.onAfterDecoded:19 - 消息解码后，channelContext: server:0.0.0.0:80, client:127.0.0.1:2855, packet: nexus.io.log.server.packet.DemoPacket@1e9344a0, packetSize: 2
...
2025-01-23 20:37:04.444 [Thread-15] INFO   c.l.l.s.l.DemoListener.onAfterHandled:31 - 消息处理后，channelContext: server:0.0.0.0:80, client:127.0.0.1:2855, packet: nexus.io.log.server.packet.DemoPacket@1e9344a0, 耗时: 0 ms
2025-01-23 20:37:04.446 [Thread-15] INFO   c.l.l.s.l.DemoListener.onAfterSent:27 - 消息发送后，channelContext: server:0.0.0.0:80, client:127.0.0.1:2855, packet: nexus.io.log.server.packet.DemoPacket@54220845, isSentSuccess: true
...
如果希望简化编码和解码的过程，不想手动处理 `ByteBuffer`，可以使用 `tio-boot` 提供的 `ByteBufferPacket` 类。它允许直接将数据写入 `ByteBuffer`，减少编码和解码的繁琐步骤。
...
- `ByteBufferPacket` 已经封装了对 `ByteBuffer` 的处理，只需要关注消息体本身。

---

**总结：**

本文介绍了如何使用 `tio-boot` 框架内置的 TCP 处理器，构建一个简单的 TCP 服务器应用。通过自定义 `Packet`、`Handler` 和 `Listener`，实现了消息的编码、解码和业务逻辑处理。并提供了详细的代码示例和解释，帮助快速上手开发。
