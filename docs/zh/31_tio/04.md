...
在 Java 中，要同时在同一个端口上处理 TCP 和 UDP，需要分别为 TCP 和 UDP 创建不同的服务端套接字，并且这两个套接字监听同一个端口号。通常，会为 TCP 使用`ServerSocket`类，并为 UDP 使用`DatagramSocket`类。
...
两个服务器（TCP 和 UDP）都在自己的线程中运行，这样它们就可以并行处理数据，并独立地接收和处理各自协议的数据。记住在实际应用中，需要适当处理异常和多线程同步等问题。
...
import nexus.io.jfinal.aop.annotation.AComponent;
...
tio-boot 同样提供了 UDP 客户端,可以使用 UDP 客户端给上面的服务发送数据

```
package demo.udp.client;

import nexus.io.tio.core.udp.UdpClient;
import nexus.io.tio.core.udp.UdpClientConf;
