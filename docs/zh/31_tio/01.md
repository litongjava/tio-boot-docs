...
`TioBootServer` 是 Tio 框架的核心启动类，负责初始化和启动 Tio 服务器。它提供了一个结构化的方式来配置和启动的服务器应用程序。在启动过程中，`TioBootServer` 将大量的类以静态变量的形式存储，使得开发者可以在框架启动前后进行灵活的配置和操作。
...
4. **`defaultHttpRequestHandlerDispather`**: 这是处理 HTTP 请求的默认分发器。它定义了如何处理和路由进来的 HTTP 请求。tio-boot 启动时会设置为 nexus.io.tio.boot.http.handler.DefaultHttpRequestHandlerDispather

5. **`defaultHttpServerInterceptorDispatcher`**: 这代表 HTTP 拦截器的分发器，可以拦截并处理 HTTP 请求和响应，tio-boot 启动时会设置为 nexus.io.tio.boot.http.interceptor.DefaultHttpServerInterceptorDispatcher

6. **`defaultWebSocketHandlerDispather`**: 这代表 websocket 的分发器，它定义了如何处理进来的 WebSocket 消息。tio-boot 启动时会设置为 nexus.io.tio.boot.websocket.handler.DefaultWebSocketHandlerDispather

7. **`serverInteceptorConfigure`**: 这是服务器拦截器的配置设置，允许为所有进来的请求或消息定义全局规则或行为。需要配合 DefaultHttpRequestHandlerDispather 使用
...
11. **`httpRoutes`**: 管理 tio http server Handler 的路由
...
上面这些熟悉都提供了静态的 get 和 set 方法,可以在需要的时候获取配置或者设置自定义的配置
