...
public class MaxBlogUserDao {
  ...
  public boolean createUser(String uid, String email, String password, Integer user_type) {
    ...
    Record record = Record.by("id", uid).set("email", email).set("password_hash", password).set("user_type", user_type);
    return Db.save(TableNames.max_blog_users, record);
  }

  public Record login(String email, String password) {
    byte[] sha1 = Sha1Utils.SHA1(password);
    password = new String(sha1);
    String sql = String.format("select id from %s where email=? and password_hash=?", TableNames.max_blog_users);
  ...
  public int updatePassword(String email, String password) {
    byte[] sha1 = Sha1Utils.SHA1(password);
    password = new String(sha1);
    String sql = String.format("update  %s set password =?  where user_email=?", TableNames.max_blog_users);
...
public class EmailService {
  ...
  public boolean sendVerificationCode(String email, Integer code) {
    ...
    try {
      MailJetUtils.sendText(name, email, "College Bot AI Email Verification", content);
      return true;
    } catch (Exception e) {
      e.printStackTrace();
      return false;
...
We received a request to reset the password for your College Bot AI account.

To reset your password, please use the following verification code:
#(code)

If you did not request a password reset, please disregard this email or contact our support team.
...
public class InterceptorConfiguration {
  ...
  public void config() {
    ...
    model.addAllowUrls("/api/v1/user/register", "/api/v1/user/login", "/api/v1/user/sendResetCode", "/api/v1/user/reset");
...
### rumi_reset_password_code

```sql
drop table if exists rumi_reset_password_code;
...
public class ResetPasswordCodeDao {
  ...
  public Integer getCode(String email, Long seconds) {
    String sql = String.format("SELECT code FROM %s  WHERE email = ?  AND create_time > NOW() - INTERVAL '%d seconds'",
        //
  ...
  public Integer newCode(String email) {
    ...
    Db.tx(() -> {
      ...
      Record record = Record.by("email", email).set("code", code);
      return Db.save(TableNames.rumi_reset_password_code, record);
    });
    return code;
  }
...
public class UserPasswordService {
  ...
  public RespBodyVo sendResetCode(String email) {
    Integer code = Aop.get(ResetPasswordCodeDao.class).getCode(email, 60L);
    if (code != null) {
      return RespBodyVo.fail("Please try again in 60 seconds");
    }
    code = Aop.get(ResetPasswordCodeDao.class).newCode(email);
    Aop.get(EmailService.class).sendVerificationCode(email, code);
    return RespBodyVo.ok();
  }
  ...
  public RespBodyVo reset(UserResetVo vo) {
    ...
    boolean ok = Aop.get(ResetPasswordCodeDao.class).validateCode(email, vo.getCode());
    if (!ok) {
      return RespBodyVo.fail("The verification code is incorrect.");
    }
    String uid = Aop.get(CollegeCoursesUsersDao.class).getUidByEmail(email);
    if (uid != null) {
      Aop.get(CollegeCoursesUsersDao.class).updatePassword(email, password);
    } else {
      return RespBodyVo.fail("Not found user email.");
    }
