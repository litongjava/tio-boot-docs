...
两者不能混用。用 `manimgl` 运行脚本时，如果写了 `from manim import *`，场景发现与运行都会出问题。
...
下面是更偏“可跑通”的配置建议。即使使用 conda，也建议把系统依赖装齐。
...
这里给出当前风格的一套流程（conda + pip）。更稳的做法是新建独立环境，避免 base 环境依赖杂糅。
...
在 SSH 会话里运行 `manimgl`，默认没有显示环境变量，最典型报错是：
...
* 如果不是 root 执行，把 `root` 换成 SSH 登录使用的用户名
...
在 ManimGL 里，如果只是要导出视频，不想开实时窗口，核心是使用 `-w` 写文件。

要求本文只介绍一种写文件方式，这里给出最终确认使用的命令格式：
...
如果的项目中输出目录不同，通常是配置或工作目录造成的，但本文以的目标路径为准。

---

## 7. 真实踩坑复盘：我遇到的问题与最终解决方案

这一节把我在真实环境里遇到的报错链路完整记录下来，便于在同类型环境里快速定位。
...
* 但运行 `manimgl` 时，进程优先加载了 conda 里的 `libstdc++.so.6`
...
如果希望以“写文件不弹窗”的方式导出，结合本文第 6 节命令即可：
...
* 一旦 conda 的这些库版本与系统图形栈编译 ABI 不匹配，就会出现遇到的这种“系统 glxinfo 正常，但 Python 程序创建 context 失败”的情况
...
* 如果想用中文字体（例如 `SimHei`、`Noto Sans CJK SC`、`WenQuanYi Micro Hei`），请确保系统已安装对应字体，否则会报字体找不到。可以用 `fc-list | grep -i` 搜索已安装字体名。
