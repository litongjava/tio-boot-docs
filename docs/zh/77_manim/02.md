...
👉 如果只需要基本 LaTeX（Manim/报告足够用），上面已经够。
👉 如果真要“full”，要自己下 ISO（>4GB）或手动安装 TeX Live，比较麻烦。
...
我看出来了，的环境是 **Aliyun Linux 3 (基于 CentOS 8/Anolis OS)**，它自带了 `epel-aliyuncs-release`，和官方 `epel-release` 冲突，所以会报错。
...
Manim 的 MathTex 走的是 latex -> dvi -> svg 的流程，默认用 \documentclass{standalone}。的 TeX Live 里缺少 standalone 宏包 会出现错误
```sh
...
```bash
conda create --name manim python=3.12 -y
conda activate manim
```
### 2.安装必要的库
安装 Manim 以及一些必要的依赖库。可以通过 pip 自动安装所需依赖（如 numpy、Pillow、opencv-python 等）。例如：
...
3. **确保完整的 LaTeX 发行版：**
   ...
   * **RHEL/CentOS/Amazon Linux（TeX Live）：**
     ...
     # 或者根据可用情况有选择地安装相应包
     ```
   * 如果是手动安装 TeX Live 或使用 `tlmgr` 管理，请确保安装 `ctex`、`fontspec` 和 `xecjk`（或对 LuaLaTeX 使用 `luatexja`）：
...
4. **Manim 配置：**
   ...
   * 确保在的 Manim 脚本中设置 `config.tex_template = TexTemplateLibrary.ctex`，这样 Manim 会配合 `ctex` 包使用 `xelatex` 或 `lualatex`。
...
如果需要 Noto Sans CJK（效果更现代，适合 LaTeX/Manim），那只能手动下载安装：

下载字体压缩包（Noto Sans CJK SC 为简体中文）：
```
mkdir /opt/package/font -p && cd /opt/package/font
wget https://noto-website-2.storage.googleapis.com/pkgs/NotoSansCJKsc-hinted-hinted.zip
```

解压到系统字体目录：
```
sudo mkdir -p /usr/share/fonts/noto-cjk
sudo unzip NotoSansCJKsc-hinted.zip -d /usr/share/fonts/noto-cjk
```
...
下面是一个名为 `binomial_square.py` 的示例代码，该代码使用 Manim 来演示上述证明过程。可以根据需要保存该文件并使用 Manim 运行它。
...
本文详细介绍了如何搭建 Manim 开发环境，包括安装 LaTeX、FFmpeg、创建 Python 环境以及安装 Manim 及其依赖。随后，通过一个入门示例展示了如何用 Manim 编写动画来证明公式 \((a+b)^2 = a^2 + 2ab + b^2\) 的几何证明过程。希望这篇文档能帮助快速上手 Manim，创作出更多生动有趣的数学动画。
