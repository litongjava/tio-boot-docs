...
本文档详细介绍了如何在 Android 设备上使用 `tio-boot` 框架运行轻量级 HTTP 服务。`tio-boot` 基于 TIO 通信协议，实现了高性能、易用的 HTTP 服务能力，非常适合嵌入式或移动端场景。以下各步将带从环境准备，到代码编写，再到最终测试，一气呵成地完成部署。
## Android平台启动tio-boot
...
dependencies {
  // Android 端视图注入和工具类
  implementation 'nexus.io:android-view-inject:1.0'
  implementation 'nexus.io:litongjava-android-utils:1.0.2'
  // 核心 HTTP 服务框架
  implementation 'nexus.io:tio-boot:2.0.1'
  // 可选：AOP 支持（如不在 Controller 中使用，可移除以节省包体积）
  implementation 'nexus.io:jfinal-aop:1.3.6'
}
```
...
```java
package nexus.io.android.tio.boot.controller;
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
  package="nexus.io.android.tio.boot">
...
import nexus.io.android.tio.boot.controller.TioBootServerApp;
import nexus.io.android.utils.acp.AcpUtils;
import nexus.io.android.utils.toast.ToastUtils;
import nexus.io.android.view.inject.annotation.FindViewByIdLayout;
import nexus.io.android.view.inject.annotation.OnClick;
import nexus.io.android.view.inject.utils.ViewInjectUtils;
...
启动后，点击界面上的启动按钮,将会在 Logcat 中看到类似输出：
...
2025-07-03 22:08:41.371 [main] INFO  TioApplicationContext.run:147 - Using cache: class nexus.io.tio.utils.cache.mapcache.ConcurrentMapCacheFactory
...
2025-07-03 22:08:41.387 [main] INFO  Threads.getTioExecutor:93 - new worker thead pool:nexus.io.tio.utils.thread.pool.SynThreadPoolExecutor@793e53c[Running, pool size = 1, active threads = 0, queued tasks = 0, completed tasks = 0]
2025-07-03 22:08:41.388 [main] INFO  TioApplicationContext.run:333 - HTTP handler:
{
  "/hi": "nexus.io.android.tio.boot.controller.-$$Lambda$yj_UDBEIbM3h-OA3eprGlvm4tbY@23912c5",
  "/hello": "nexus.io.android.tio.boot.controller.-$$Lambda$H2BQOHvwVwZqr4LaO93a0lg1uWk@f63341a"
}
2025-07-03 22:08:41.403 [main] INFO  TioBootHttpControllerRouter.printMapping:454 - controller method mapping
{
  "": "nexus.io.android.tio.boot.controller.IndexController.index()",
  "/student": "nexus.io.android.tio.boot.controller.IndexController.student()"
}
...
implementation 'nexus.io:jfinal-aop:1.3.7' // jfinal‑aop 本身
```
