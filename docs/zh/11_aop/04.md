...
- 可以在其他项目中使用 jfinal-aop
- 也可以在 Android 工程中使用 jfinal-aop
...
`Cat` 类代表了一个简单的实体，包含一个方法 `eat()`，该方法通过 `@AopBefore` 注解与 `Aspect1` 类关联，表示在调用 `eat()` 方法前，应先执行 `Aspect1` 拦截器的逻辑。
...
- 使用 `AopInvocation` 对象获取被拦截方法的相关信息（方法本身、方法名、参数、目标对象）并打印。
- 执行实际的方法调用（`AopInvocation.invoke()`）。
...
public class Aspect1 implements Interceptor {
  ...
  public void intercept(AopInvocation AopInvocation) {
    ...
    Method method = AopInvocation.getMethod();
    String methodName = AopInvocation.getMethodName();
    Object[] args = AopInvocation.getArgs();
    Object target = AopInvocation.getTarget();
    log.info("method:{}", method);
    log.info("methodName:{}", methodName);
    log.info("args:{}", Arrays.toString(args));
    log.info("target:{}", target);
    Object invoke = AopInvocation.invoke();
    AopInvocation.setReturnValue("set new value");
...
import nexus.io.jfinal.aop.annotation.AController;
